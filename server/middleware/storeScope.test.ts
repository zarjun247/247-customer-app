...
describe("requireStoreAccessForUserAtStore", () => {
  it("allows super_admin to access any store", () => {
    expect(() => requireStoreAccessForUserAtStore(makeCtx("super_admin", null), 99)).not.toThrow();
  });

  it("allows staff assigned to matching store", () => {
    expect(() => requireStoreAccessForUserAtStore(makeCtx("pharmacist", 5), 5)).not.toThrow();
  });

  it("blocks staff assigned to a different store", () => {
    expect(() => requireStoreAccessForUserAtStore(makeCtx("pharmacist", 5), 7)).toThrow();
  });
