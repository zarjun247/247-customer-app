...
import { MemoryRateLimitStore, buildRateLimitKey, type RateLimitPolicy, type RateLimitStore } from "../services/rateLimitService";
...
export const PROCEDURE_RATE_LIMITS: ReadonlyMap<string, RateLimitPolicy> = new Map([
...
export type RateLimitMiddlewareContext = {
  ...
  req?: { ip?: string; socket?: { remoteAddress?: string }; headers?: Record<string, string | string[] | undefined> } | null;
};
...
export function checkProcedureRateLimit(
  ...
  const key = buildRateLimitKey({ procedure: path, ip, userId: userId ?? "anon" });
...
export type RouteRateLimitConfig = { windowMs: number; max: number; blockMs?: number };
...
export function rateLimitAuthRoutes(store: RateLimitStore = defaultRateLimitStore): RequestHandler {
  return createExpressRateLimitMiddleware("/api/auth", { windowMs: 60_000, max: 10, blockMs: 300_000 }, store);
}

export function rateLimitWebhookRoutes(store: RateLimitStore = defaultRateLimitStore): RequestHandler {
  return createExpressRateLimitMiddleware("/api/webhooks", { windowMs: 60_000, max: 300 }, store);
}
...
export function throwIfRateLimited(
  ...
  const { limited, retryAfterMs } = checkProcedureRateLimit(path, ctx, store, config);
  if (limited) {
    throw new TRPCError({
