...
import {
  ...
  requestErrorLoggerMiddleware,
  buildRequestLogEntry,
} from "./requestLogger";
import { buildCspMiddleware, parseCspMode } from "./cspEnforcer";
import { csrfMiddleware, handleCsrfError } from "./csrf";
...
export const rateLimitScaffold: RateLimitPolicy[] = [
  {
    ...
    notes:
      "OTP send/verify must use OTP_RATE_LIMIT_BACKEND in production; existing OTP guard fails closed for provider-backed OTP.",
  },
  {
    id: "login-auth",
    ...
    productionBackendRequired: true,
    notes:
      "Login/auth attempts require a shared backend before multi-instance production.",
  },
  {
    ...
    productionBackendRequired: true,
    notes:
      "Upload pressure must be limited per user/IP and paired with route-specific body limits.",
  },
  {
    ...
    productionBackendRequired: true,
    notes:
      "Cart/checkout mutations need per-user/IP throttles to protect stock reservation boundaries.",
  },
  {
    ...
    productionBackendRequired: true,
    notes:
      "Search endpoints need abuse controls without changing catalog business rules.",
  },
  {
    id: "webhook-provider",
    ...
    notes:
      "Provider webhooks should prefer signature verification and provider retry semantics over blunt blocking.",
  },
];
...
export function corsAllowlistMiddleware(
  ...
  return (req, res, next) => {
    ...
    res.setHeader(
      "Vary",
      [res.getHeader("Vary"), "Origin"].filter(Boolean).join(", ")
    );
    if (origin && isAllowed) {
...
export function registerRawWebhookParsers(app: MiddlewareRegistrar) {
  app.use(
    ...
    express.raw({ type: "application/json", limit: RAW_WEBHOOK_LIMIT })
  );
  app.use(
...
export function applyHttpSecurity(
  ...
  app.use(
    buildCspMiddleware(parseCspMode(env.CSP_MODE), env.CSP_REPORT_URI ?? null)
  );
