...
describe('stock truth 10 static guards', () => {
  it('scan routes remain lookup-only (no stock movement writes)', () => {
    const files = fs.readdirSync('server/routers').filter(f=>f.endsWith('.ts')).map(f=>`server/routers/${f}`);
  ...
  it('routers avoid direct stock movement inserts', () => {
    ...
    const found = files.some(f=> pattern.test(read(f)) && !/legacy|deferred/.test(read(f)));
    expect(found).toBe(false);
  });
