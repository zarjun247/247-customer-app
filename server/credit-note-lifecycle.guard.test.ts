...
function makeDb(state: {
  ...
  return {
    ...
    update: (table: unknown) => ({
      set: (patch: unknown) => ({
        where: async () => {
          if (table === creditNotes) Object.assign(notes[0] as object, patch);
        },
      }),
    }),
    notes,
...
describe("credit note lifecycle guards", () => {
  ...
  it("blocks a credit note that exceeds the original refundable invoice amount", () => {
    expect(() =>
      assertCreditNoteAmountAllowed({
        invoiceAmountPaise: 10000,
        requestedAmountPaise: 10001,
      })
    ).toThrow("Credit note exceeds refundable invoice amount");
  });

  it("blocks multiple issued credit notes from exceeding the original invoice amount", () => {
    expect(() =>
      assertCreditNoteAmountAllowed({
        invoiceAmountPaise: 20000,
        existingIssuedCreditNotePaise: 15000,
        requestedAmountPaise: 6000,
      })
    ).toThrow("Credit note exceeds refundable invoice amount");
  });

  it("derives safe sale return linkage and GST summary without inventing missing line detail", () => {
    const summary = deriveCreditNoteSummaryFromReturn(
      { totalRefund: "59.00", gstReversal: "9.00" },
      [
        {
          saleLineId: "line-2",
          productId: "prod-2",
    ...
    expect(summary).toMatchObject({
      amountPaise: 5900,
      taxableAmountPaise: 5000,
      gstAmountPaise: 900,
    });
    expect(summary.lineSplits[0]).toMatchObject({
      saleLineId: "line-2",
      grossAmountPaise: 5900,
    });
  });

  it("allows optional real refund ids but rejects placeholder refund-ledger linkage", () => {
    expect(() =>
      assertNoFakeRefundLedgerLink("rfnd_current_main_1")
    ).not.toThrow();
    expect(() => assertNoFakeRefundLedgerLink(null)).not.toThrow();
    expect(() =>
      assertNoFakeRefundLedgerLink("refund-ledger-pending-1")
    ).toThrow("Do not fake refund ledger linkage");
  });
  ...
  it("issues draft notes with optional current-main refund linkage and no refund ledger fabrication", async () => {
    ...
    const issued = await issueCreditNote(
      { creditNoteId: "draft-1", issuedBy: "manager-1" },
      db
    );
