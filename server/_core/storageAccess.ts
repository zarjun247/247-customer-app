...
const ALLOWED_PREFIXES = ["prescriptions/", "invoices/", "reports/", "product-images/", "uploads/"];
const SENSITIVE_PREFIXES = ["prescriptions/", "invoices/", "reports/"];

export function assertSafeStorageKey(key: string): void {
  if (!key || key.includes("..") || key.includes("\\") || key.startsWith("/")) throw new Error("unsafe key");
  if (!ALLOWED_PREFIXES.some(p => key.startsWith(p))) throw new Error("prefix not allowed");
...
export function canAccessStorageKey(user: { id: number; role?: string } | null, key: string): boolean {
