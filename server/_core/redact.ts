export function redactSensitive(input: string): string {
  return input
    .replace(
      /(["\']?(?:otp|code)["\']?\s*[:=]\s*["\']?)\d{4,8}/gi,
      "$1[REDACTED]"
    )
    .replace(/bearer\s+[a-z0-9\-._~+/]+=*/gi, "Bearer [REDACTED]")
    .replace(
      /(["\']?(?:authorization|token|signature|secret|gatewaySignature)["\']?\s*[:=]\s*["\']?)[^\s,}"]+/gi,
      "$1[REDACTED]"
    )
    .replace(
    ...
    .replace(
      /(["\']?(?:prescriptionImage|imageData|rawImage|image)["\']?\s*[:=]\s*["\']?)data:image\/[a-z0-9.+-]+;base64,[a-z0-9+/=]+/gi,
      "$1[REDACTED]"
    )
    .replace(
