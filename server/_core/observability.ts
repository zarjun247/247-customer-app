...
const queueBacklog = new Gauge({ name: "queue_backlog", help: "OCR queue backlog derived from worker runtime", registers: [register] });
const workerProcessed = new Counter({ name: "worker_processed_total", help: "Processed worker jobs", registers: [register] });
const workerJobBacklog = new Gauge({ name: "worker_job_backlog", help: "Queued/retry worker jobs from worker_jobs", registers: [register] });
const workerJobDeadLetter = new Gauge({ name: "worker_job_dead_letter_total", help: "Dead-letter worker jobs from worker_jobs", registers: [register] });
const providerWebhookEventsTotal = new Gauge({ name: "provider_webhook_events_total", help: "Provider webhook events from provider_webhook_events", registers: [register] });
const providerWebhookFailedTotal = new Gauge({ name: "provider_webhook_failed_total", help: "Failed provider webhook events from provider_webhook_events", registers: [register] });
const providerRetryScheduledTotal = new Gauge({ name: "provider_retry_scheduled_total", help: "Provider webhook events scheduled for retry from provider_webhook_events", registers: [register] });
const providerDeadLetterTotal = new Gauge({ name: "provider_dead_letter_total", help: "Provider dead letters from provider_dead_letters", registers: [register] });
const providerLatency = new Histogram({ name: "provider_latency_seconds", help: "Provider latency seconds recorded by runtime provider integrations", buckets: [0.005, 0.01, 0.05, 0.1, 0.5, 1, 2, 5], registers: [register] });
...
export function buildHttpRequestLog(req: Request, res: Response, latency: number) {
  return safeMetadata({
    event: "http_request",
    method: req.method,
...
export function initObservability(app: Express) {
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    res.on("finish", () => {
      ...
      (req as any).log.info(buildHttpRequestLog(req, res, latency), "http_request");
    });
  ...
  logger.info({ event: "observability_initialized" }, "observability_initialized");
}
