...
function createFakeDb() {
  ...
  return {
    ...
    async findTallyExportRun(input: any) {
      return runs.find((run) => run.storeId === input.storeId && run.exportType === input.exportType && run.checksum === input.checksum && String(run.periodStart) === String(input.periodStart) && String(run.periodEnd) === String(input.periodEnd)) ?? null;
    },
...
const ledgerRows = [
  { id: 1, storeId: 7, sourceType: "sale", sourceId: 101, entryDate: new Date("2026-04-01T10:00:00Z"), accountCode: "SALES", accountName: "Sales", debit: 0, credit: "100.00", narration: "Invoice INV-101" },
  { id: 2, storeId: 7, sourceType: "payment", sourceId: 101, entryDate: new Date("2026-04-01T10:00:00Z"), accountCode: "CASH", accountName: "Cash", debit: "100.00", credit: 0, narration: "Paid INV-101" },
  { id: 3, storeId: 7, sourceType: "gst_output", sourceId: 101, entryDate: new Date("2026-04-01T10:00:00Z"), accountCode: "OUTPUT_GST", accountName: "Output GST", debit: 0, credit: "12.00", narration: "GST INV-101" },
];

describe("tally export proof", () => {
  it("creates an export run row and never claims import/sync", async () => {
    const db = createFakeDb();
    const result = await generateTallyCsvExport(db, { exportType: "journal_csv", rows: ledgerRows, storeId: 7, periodStart: new Date("2026-04-01T00:00:00Z"), periodEnd: new Date("2026-04-30T23:59:59Z"), generatedBy: 9 });
  ...
  it("blocks a duplicate export for store/type/period/checksum", async () => {
    const db = createFakeDb();
    const base = { exportType: "journal_csv", rows: ledgerRows, storeId: 7, periodStart: new Date("2026-04-01T00:00:00Z"), periodEnd: new Date("2026-04-30T23:59:59Z") };
  ...
  it("exports debit and credit CSV rows with GST ledger mapping", () => {
    const rows = buildTallyCsvRows(ledgerRows, { companyIdentifier: "STORE-7" });
    ...
    expect(defaultTallyLedgerMapping({ sourceType: "gst_input", accountCode: "INPUT_GST", accountName: "Input GST" })).toBe("Input GST");
  });

  it("persists failed status with failure reason when generation fails", async () => {
    const db = createFakeDb();
    const result = await generateTallyCsvExport(db, { exportType: "journal_csv", rows: [{ id: 99, debit: 0, credit: 0 }], storeId: 7 });
  ...
  it("documents export proof migration and schema fields", () => {
    const migration = fs.readFileSync("drizzle/0040_tally_export_proof.sql", "utf8");
