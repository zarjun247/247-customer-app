...
import {
  commitPurchaseInvoiceExactlyOnce,
  confirmSaleExactlyOnce,
  settleProviderRefundExactlyOnce,
} from "./services/commercialTruthSeams";
...
if (!testDatabaseUrl) {
  console.warn(
    `Skipping MySQL concurrency integration proof because ${TEST_DB_ENV_VAR} is not set. DB-backed race proof is not claimed.`
  );
}
...
async function seedConcurrencyFixture(ctx: DbTestContext, suffix: string) {
  ...
  await ctx.connection.execute(
    "UPDATE batches SET qtyOnHand = 1, quantity = 1, qtyReserved = 0 WHERE id = ?",
    [productSet.batch.id]
  );
  await ctx.connection.execute(
    "UPDATE store_skus SET stockQty = 1, softLockedQty = 0 WHERE id = ?",
    [productSet.storeSku.id]
  );
  ...
  const customerB = await insertId(
    ...
    [
      `test-customer-b-${fixtureRunId}`,
      "Test Customer B",
      `customer-b-${fixtureRunId}@example.test`,
      "+910000000003",
    ]
  );
...
async function cleanupConcurrencyRows(ctx: DbTestContext) {
  await ctx.connection.execute(
    "DELETE FROM idempotency_keys WHERE `key` LIKE ? OR entityId LIKE ?",
    [`%${ctx.runId}%`, `%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM stock_movements WHERE reason LIKE ?",
    [`%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM h1_register WHERE saleRef LIKE ? OR saleLineRef LIKE ? OR prescriptionRef LIKE ?",
    [`%${ctx.runId}%`, `%${ctx.runId}%`, `%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM provider_webhook_events WHERE idempotencyKey LIKE ? OR providerEventId LIKE ?",
    [`%${ctx.runId}%`, `%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM payment_records WHERE gatewayOrderId LIKE ? OR gatewayPaymentId LIKE ?",
    [`%${ctx.runId}%`, `%${ctx.runId}%`]
  );
  await ctx.connection.execute(
  ...
  await ctx.connection.execute(
    "DELETE FROM counter_payments WHERE id LIKE ? OR sale_id LIKE ?",
    [`%${ctx.runId}%`, `%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM invoice_snapshots WHERE bill_no LIKE ? OR sale_id LIKE ?",
    [`%${ctx.runId}%`, `%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM sale_lines WHERE id LIKE ? OR sale_id LIKE ?",
    [`%${ctx.runId}%`, `%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM sales WHERE id LIKE ? OR bill_no LIKE ?",
    [`%${ctx.runId}%`, `%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM stock_reservations WHERE releaseReason LIKE ?",
    [`%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM order_items WHERE orderId IN (SELECT id FROM orders WHERE statusReason LIKE ?)",
    [`%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM orders WHERE statusReason LIKE ? OR deliveryAddress LIKE ?",
    [`%${ctx.runId}%`, `%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    "DELETE FROM purchase_lines WHERE batchNo LIKE ?",
    [`%${ctx.runId}%`]
  );
  await ctx.connection.execute(
  ...
  await ctx.connection.execute(
    "DELETE FROM batch_ledger WHERE batchNo LIKE ?",
    [`%${ctx.runId}%`]
  );
  await ctx.connection.execute(
    ...
    if (update.affectedRows === 1) {
      await connection.execute(
        ...
        [
          seed.productSet.batch.id,
          orderId,
          seed.productSet.product.id,
          seed.productSet.variant.id,
          seed.productSet.storeSku.id,
          seed.store.id,
          `mysql-concurrency-${runId}`,
        ]
      );
    }
...
async function createSaleConfirmFixture(ctx: DbTestContext, seed: Seed) {
  ...
  await ctx.connection.execute(
    ...
    [
      saleId,
      `DRF-${ctx.runId}`,
      String(seed.store.id),
      String(seed.staff.id),
      Date.now(),
      Date.now(),
    ]
  );
  await ctx.connection.execute(
    ...
    [
      `ln-${ctx.runId.slice(-12)}`,
      saleId,
      String(seed.productSet.product.id),
      String(ledgerId),
      `SL-${ctx.runId}`,
      Date.now(),
    ]
...
describeWithDb("MySQL-backed concurrency proof harness", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createDbTestContext(
      `mysql_concurrency_${Date.now()}_${process.pid}`
    );
  ...
  afterAll(async () => {
    ...
    if (previousWebhookEnabled === undefined)
      delete process.env.PAYMENT_WEBHOOK_ENABLED;
    else process.env.PAYMENT_WEBHOOK_ENABLED = previousWebhookEnabled;
  ...
  it("proves the last-unit reservation race is protected by an atomic DB predicate", async () => {
    ...
    await ctx.connection.execute(
      "UPDATE orders SET statusReason = ?, deliveryAddress = ? WHERE id IN (?, ?)",
      [ctx.runId, ctx.runId, seed.orderA, seed.orderB]
    );

    const outcomes = await Promise.all([
      reserveOneUnit(seed, ctx.runId, seed.orderA),
      reserveOneUnit(seed, ctx.runId, seed.orderB),
    ]);
    ...
    const [batch] = await ctx.db
      .select()
      .from(batches)
      .where(eq(batches.id, seed.productSet.batch.id))
      .limit(1);
    ...
    const reservations = await ctx.db
      ...
      .where(eq(stockReservations.batchId, seed.productSet.batch.id));
  ...
  it("proves a POS sale and app reservation cannot both consume the same last SKU/batch unit", async () => {
    ...
    expect(
      (batch.qtyOnHand ?? 0) - (batch.qtyReserved ?? 0)
  ...
  it("proves concurrent invoice number reservations remain unique through the existing invoiceNumbering service", async () => {
    ...
    const reserveWithIsolatedConnection = async () => {
      const connection = await openTestConnection(testDatabaseUrl);
      try {
        return await reserveInvoiceNumber(
    ...
    const invoices = await Promise.all(
      Array.from({ length: 12 }, () => reserveWithIsolatedConnection())
    );
    ...
    const rows = await ctx.db
      ...
      .where(eq(invoiceSequences.storeId, storeRef));
  ...
  it("proves duplicate H1 sale-line registration is rejected by the H1 register unique key", async () => {
    ...
    const rows = await ctx.db
      .select()
      .from(h1Register)
      .where(eq(h1Register.saleRef, saleRef));
    expect(rows).toHaveLength(1);
  });
  ...
  it("proves purchase commit double-submit through the exported idempotent service seam", async () => {
    ...
    const outcomes = await Promise.allSettled([
      commitPurchaseInvoiceExactlyOnce({
        invoiceId,
        idempotencyKey: `purchase:${ctx.runId}`,
        actorId: seed.staff.id,
        actorRole: seed.staff.role,
      }),
      commitPurchaseInvoiceExactlyOnce({
        invoiceId,
        idempotencyKey: `purchase:${ctx.runId}`,
    ...
    const movements = await ctx.db
      .select()
      .from(stockMovements)
      .where(eq(stockMovements.referenceId, invoiceId));
    expect(movements).toHaveLength(1);
    expect(movements[0].qty).toBe(3);
    const idemRows = await ctx.db
      .select()
      .from(idempotencyKeys)
      .where(eq(idempotencyKeys.key, `purchase:${ctx.runId}`));
  ...
  it("proves duplicate supplier invoice commit is blocked non-destructively before hard uniqueness", async () => {
    ...
    const outcome = await Promise.allSettled([
      commitPurchaseInvoiceExactlyOnce({
        invoiceId,
        idempotencyKey: `purchase:dup:${ctx.runId}`,
        actorId: seed.staff.id,
        actorRole: seed.staff.role,
      }),
    ]);
    ...
    if (outcome[0].status === "rejected") {
      expect(String(outcome[0].reason?.message ?? outcome[0].reason)).toContain(
  ...
  it("proves sale confirmation double-submit through the exported idempotent service seam", async () => {
    ...
    const [ledger] = await ctx.db
      .select()
      .from(batchLedger)
      .where(eq(batchLedger.id, ledgerId))
      .limit(1);
  ...
  it("proves full payment state-transition replay settles payment/order once through the webhook seam", async () => {
    ...
    await ctx.connection.execute(
      "UPDATE orders SET statusReason = ?, deliveryAddress = ? WHERE id = ?",
      [ctx.runId, ctx.runId, seed.orderA]
    );
    const gatewayOrderId = `order_${ctx.runId}`;
    await ctx.connection.execute(
      "INSERT INTO payment_records (orderId, userId, gatewayOrderId, amount, currency, status) VALUES (?, ?, ?, 9500, 'INR', 'pending')",
      [seed.orderA, seed.customerA.id, gatewayOrderId]
    );
    const rawBody = JSON.stringify({
      event: "payment.captured",
      id: `evt_pay_${ctx.runId}`,
      payload: {
        payment: {
          entity: {
            id: `pay_${ctx.runId}`,
            order_id: gatewayOrderId,
            notes: { orderId: String(seed.orderA) },
          },
        },
      },
    });
    ...
    const events = await ctx.db
      ...
      .where(eq(providerWebhookEvents.providerEventId, `evt_pay_${ctx.runId}`));
  ...
  it("proves concurrent refund settlement cannot exceed paid amount and provider refund replay is harmless", async () => {
    ...
    await ctx.connection.execute(
      "INSERT INTO payment_records (orderId, userId, gatewayOrderId, gatewayPaymentId, amount, currency, status, paidAt) VALUES (?, ?, ?, ?, 1000, 'INR', 'paid', NOW())",
      [
        seed.orderA,
        seed.customerA.id,
        gatewayOrderId,
        `pay_refund_${ctx.runId}`,
      ]
    );
    const outcomes = await Promise.allSettled([
      settleProviderRefundExactlyOnce({
        gatewayOrderId,
        providerRefundId: `rfnd_a_${ctx.runId}`,
        amountPaise: 700,
        idempotencyKey: `refund:a:${ctx.runId}`,
        actorId: seed.staff.id,
      }),
      settleProviderRefundExactlyOnce({
        gatewayOrderId,
        providerRefundId: `rfnd_b_${ctx.runId}`,
        amountPaise: 700,
        idempotencyKey: `refund:b:${ctx.runId}`,
        actorId: seed.staff.id,
      }),
    ]);
    ...
    const duplicate = await settleProviderRefundExactlyOnce({
      gatewayOrderId,
      providerRefundId: winningProviderRefundId,
      amountPaise: 700,
      idempotencyKey: `refund:replay:${ctx.runId}`,
      actorId: seed.staff.id,
    });
    expect(duplicate.idempotent).toBe(true);
    expect(
      allRefunds.reduce((sum, row) => sum + Number(row.amountPaise), 0)
  ...
  it("proves reservation expiry during payment has one deterministic terminal winner", async () => {
    ...
    const outcomes = await Promise.all([
      claimReservationTerminalState({
        id: reservationId,
        terminalStatus: "consumed",
        releaseReason: `payment_claim_${ctx.runId}`,
      }),
      claimReservationTerminalState({
        id: reservationId,
        terminalStatus: "expired",
        releaseReason: `expiry_claim_${ctx.runId}`,
      }),
    ]);
