...
const paymentLifecycle = readFileSync("server/services/paymentWebhookLifecycle.ts", "utf8");
...
describe("router parity commercial truth seams", () => {
  it("purchase commit delegates runtime mutation to commitPurchaseInvoiceExactlyOnce", () => {
    const block = purchaseRouter.slice(purchaseRouter.indexOf("commitInvoice"), purchaseRouter.indexOf("stockMovements"));
  ...
  it("sale confirmation delegates runtime mutation to confirmSaleExactlyOnce", () => {
    const block = salesRouter.slice(salesRouter.indexOf("confirmSale: protectedProcedure"), salesRouter.indexOf("getInvoiceSnapshotPackage: protectedProcedure"));
