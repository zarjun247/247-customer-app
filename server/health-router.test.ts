...
vi.mock("./services/healthcheck", () => ({
  publicLiveness: vi.fn(() => ({ status: "ok", timestamp: "2026-05-09T00:00:00.000Z" })),
  publicReadiness: vi.fn(async () => ({ status: "ready", timestamp: "2026-05-09T00:00:00.000Z", checks: { database: "healthy", migrations: "healthy" } })),
  getHealthReport: vi.fn(async () => ({ status: "healthy", generatedAt: "2026-05-09T00:00:00.000Z", app: { status: "healthy", uptimeSeconds: 1, environment: "test" }, database: { status: "healthy" }, migrations: { status: "healthy" }, providers: { razorpay: { status: "not_configured" } }, workerQueue: { status: "configured" }, stockReservationSanity: { status: "healthy" } })),
}));
...
describe("health router", () => {
  ...
  it("/healthz and /api/healthz are minimal and secret-free", async () => {
    await withServer(async (baseUrl) => {
      for (const path of ["/healthz", "/api/healthz"]) {
        ...
        expect(body).toEqual({ status: "ok", timestamp: "2026-05-09T00:00:00.000Z" });
  ...
  it("/readyz and /api/readyz are secret-free and non-2xx when critical checks fail", async () => {
    vi.mocked(healthModule.publicReadiness).mockResolvedValueOnce({ status: "not_ready", timestamp: "2026-05-09T00:00:00.000Z", checks: { database: "unhealthy", migrations: "healthy" } } as any);
    await withServer(async (baseUrl) => {
