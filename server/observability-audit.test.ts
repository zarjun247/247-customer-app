...
vi.mock("./_core/sdk", () => ({ sdk: { authenticateRequest: vi.fn(async () => { throw new Error("no session"); }) } }));
vi.mock("./db", () => ({ getDb: vi.fn(async () => null) }));

const sdkModule = await import("./_core/sdk");
const { buildHttpRequestLog, initObservability } = await import("./_core/observability");
const { registerObservabilityRoutes } = await import("./routers/observabilityRouter");
const { OBSERVABILITY_METRIC_NAMES, getProviderVisibilitySummary } = await import("./services/operationalVisibility");

async function withServer<T>(configure: (app: express.Express) => void, fn: (baseUrl: string) => Promise<T>): Promise<T> {
  const app = express();
  configure(app);
...
describe("observability audit guards", () => {
  ...
  it("gates observability JSON routes behind staff/admin authentication", async () => {
    ...
    await withServer((app) => registerObservabilityRoutes(app), async (baseUrl) => {
      const forbidden = await fetch(`${baseUrl}/api/observability/dashboards`);
      expect(forbidden.status).toBe(404);

      vi.mocked(sdkModule.sdk.authenticateRequest).mockResolvedValueOnce({ id: 1, role: "admin" } as any);
      const allowed = await fetch(`${baseUrl}/api/observability/dashboards`);
      expect(allowed.status).toBe(200);
      const body = await allowed.json();
      expect(Array.isArray(body.dashboards)).toBe(true);
      expect(body.supportedMetrics).toEqual(expect.arrayContaining(["provider_dead_letter_total"]));
    });
  });

  it("gates /metrics and exposes the expected Prometheus metric shape", async () => {
    process.env.NODE_ENV = "production";
    await withServer((app) => initObservability(app), async (baseUrl) => {
      const forbidden = await fetch(`${baseUrl}/metrics`);
      expect(forbidden.status).toBe(404);

      vi.mocked(sdkModule.sdk.authenticateRequest).mockResolvedValueOnce({ id: 2, role: "store_manager" } as any);
  ...
  it("keeps dashboard definitions free of fake or unsupported metric claims", () => {
    const supported = new Set<string>(OBSERVABILITY_METRIC_NAMES as readonly string[]);
    const dashboardDir = path.join(process.cwd(), "docs", "dashboards");
    const dashboards = fs.readdirSync(dashboardDir).filter((file) => file.endsWith(".json"));
    ...
    for (const file of dashboards) {
      ...
      expect(JSON.stringify(dashboard).toLowerCase()).not.toContain("placeholder");
