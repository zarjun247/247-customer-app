...
function expectBalanced(
  batch: Parameters<typeof assertBalancedJournalBatch>[0]
) {
...
describe("balanced accounting journal batches", () => {
  it("posts a balanced sale helper with output GST credit", () => {
    const batch = createSaleJournalBatch({
      saleId: 101,
      storeId: 1,
      total: 112,
      gstAmount: 12,
    ...
    expect(batch.lines).toContainEqual(
      expect.objectContaining({ accountName: "Output GST", credit: 12 })
    );
    expectBalanced(batch);
  });

  it("posts a balanced purchase helper with input GST debit", () => {
    const batch = createPurchaseJournalBatch({
      purchaseInvoiceId: 202,
      storeId: 1,
      netAmount: 118,
      totalGst: 18,
    });
    expect(batch.lines).toContainEqual(
      expect.objectContaining({ accountName: "Input GST", debit: 18 })
    );
    expectBalanced(batch);
  });
  ...
  it("trial balance is limited to posted batches and detects legacy/orphan rows", () => {
    ...
    expect(service).toContain(
      "isNull(accountingJournalEntries.journalBatchId)"
    );
  });
