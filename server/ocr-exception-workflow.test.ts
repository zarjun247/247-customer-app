...
describe("OCR invoice exception workflow", () => {
  ...
  it("clean invoice line maps without exception but still requires human approval before handoff", () => {
    ...
    expect(() =>
      assertOcrDraftApprovedForHandoff({ status: "draft" }, [
        {
          ...cleanLine,
          productId: 101,
          status: "pending",
  ...
  it("routes ambiguous product to exception", () => {
    expect(classifyOcrLineException({ ...cleanLine, candidateCount: 2 })).toBe(
      "ambiguous_product"
    );
  });

  it("routes low confidence to exception", () => {
    expect(classifyOcrLineException({ ...cleanLine, confidence: 45 })).toBe(
      "low_confidence"
    );
  });

  it("routes missing batch to exception", () => {
    expect(classifyOcrLineException({ ...cleanLine, batchNo: "" })).toBe(
      "missing_batch"
    );
  });

  it("routes missing expiry to exception", () => {
    expect(classifyOcrLineException({ ...cleanLine, expiryDate: null })).toBe(
      "missing_expiry"
    );
  });

  it("routes missing MRP/cost to exception", () => {
    expect(classifyOcrLineException({ ...cleanLine, mrp: 0 })).toBe(
      "missing_mrp"
    );
    expect(classifyOcrLineException({ ...cleanLine, purchaseRate: 0 })).toBe(
      "missing_cost"
    );
  });

  it("routes missing HSN/GST and regulated schedule gaps to exceptions", () => {
    expect(classifyOcrLineException({ ...cleanLine, hsnCode: "" })).toBe(
      "missing_hsn_or_gst"
    );
    expect(
      classifyOcrLineException({
        ...cleanLine,
        regulatedRequiresSchedule: true,
        schedule: null,
      })
    ).toBe("missing_schedule_for_regulated");
  });
  ...
  it("routes supplier SKU below confidence threshold to exception", () => {
    expect(
      classifyOcrLineException({
        ...cleanLine,
        supplierSku: "SUP-1",
        mappedSupplierSkuId: null,
        matchConfidence: 80,
      })
    ).toBe("supplier_sku_unmapped");
  });

  it("approved OCR draft can hand off to existing purchase commit path", () => {
    expect(() =>
      assertOcrDraftApprovedForHandoff({ status: "approved" }, [
        {
          ...cleanLine,
          productId: 101,
          status: "approved",
  ...
  it("OCR flow does not directly call stock mutation/invariant except through approved purchase handoff", () => {
    ...
    const pattern =
      /increaseStockForPurchaseCommit|applyStockMovement|insert\(stockMovements\)|update\(batchLedger\)|syncStoreSkuAggregate/;
