...
import { reserveInvoiceNumber, formatInvoiceNumber } from "./services/invoiceNumbering";
...
describe("immutable invoice snapshots", () => {
  ...
  it("wires snapshot creation into sale and order invoice generation paths", () => {
    const salesRouter = readFileSync(new URL("./routers/salesRouter.ts", import.meta.url), "utf8");
    ...
    expect(dbModule).toContain("createOrderInvoiceSnapshot(db, orderId, { pdfFileKey: key, pdfFileUrl: url }");
    expect(dbModule).toContain("createOrderInvoiceSnapshot(db, orderId, { failureReason:");
  });
  ...
  it("creates a statutory-complete payload with GST/HSN/MRP/discount/taxable values", () => {
    const payload = buildSaleInvoiceSnapshotPayload({
      sale,
      lines: lineRows,
      store: { name: "24/7 Pharmacy", address: "Main Road", gstin: "27ABCDE1234F1Z5", drugLicense: "MH-DL-123" },
      payment: { paymentMode: "upi", paymentRef: "upi-123", status: "confirmed", amount: "201.60" },
  ...
  it("surfaces missing statutory store fields instead of faking completeness", () => {
    const payload = buildSaleInvoiceSnapshotPayload({ sale, lines: lineRows, store: { name: "24/7 Pharmacy" } });
    expect(payload.statutoryCompleteness.complete).toBe(false);
    expect(payload.statutoryCompleteness.status).toBe("warning");
    expect(payload.statutoryCompleteness.missingFields).toEqual(expect.arrayContaining(["storeGSTIN", "storeDrugLicense", "storeAddress"]));
  });
  ...
  it("remains immutable when product, store, or customer master data changes later", () => {
    const payload = buildSaleInvoiceSnapshotPayload({
      sale,
      lines: lineRows,
      store: { name: "Original Store", address: "Original Address", gstin: "27ABCDE1234F1Z5", drugLicense: "MH-DL-123" },
    });
    ...
    const changedPayload = buildSaleInvoiceSnapshotPayload({
      ...
      lines: [{ ...lineRows[0], productName: "Changed Product", line: { ...lineRows[0].line, saleRate: "999.00" } }],
      store: { name: "Changed Store", address: "Changed Address", gstin: "99ABCDE1234F1Z5", drugLicense: "NEW-DL" },
    });
  ...
  it("does not claim PDF success without both PDF key and URL", () => {
    const payload = buildSaleInvoiceSnapshotPayload({ sale, lines: lineRows, store: { name: "24/7 Pharmacy" } });
    const row = { snapshotJson: payload, pdfFileKey: null, pdfFileUrl: null, status: "generated" };
  ...
  it("builds insurer-ready package without claim submission automation", () => {
    ...
    expect(pkg.saleReference).toMatchObject({ saleId: "sale-1", status: "confirmed" });
  ...
  it("prevents a customer from accessing another customer's invoice snapshot", () => {
    const payload = buildSaleInvoiceSnapshotPayload({ sale, lines: lineRows, store: { name: "24/7 Pharmacy" } });
    expect(assertCustomerCanAccessInvoiceSnapshot({ customerId: "101", snapshotJson: payload }, { id: 101, role: "customer" })).toBe(true);
    expect(assertCustomerCanAccessInvoiceSnapshot({ customerId: "101", snapshotJson: payload }, { id: 202, role: "customer" })).toBe(false);
    expect(assertCustomerCanAccessInvoiceSnapshot({ customerId: "101", snapshotJson: payload }, { id: 202, role: "admin" })).toBe(true);
  });
