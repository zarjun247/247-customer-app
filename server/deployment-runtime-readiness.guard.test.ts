...
describe("deployment runtime readiness routers", () => {
  ...
  it("staff/admin gates runtime visibility endpoints", async () => {
    const customerCaller = appRouter.createCaller(
      baseCtx("customer") as unknown
    );
    await expect(
      customerCaller.multiStoreRuntime.overview()
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      customerCaller.deploymentReadiness.summary()
    ).rejects.toMatchObject({ code: "FORBIDDEN" });

    const staffCaller = appRouter.createCaller(baseCtx("admin") as unknown);
    await expect(
      staffCaller.multiStoreRuntime.overview()
    ).resolves.toMatchObject({ status: "unknown" });
    await expect(
      staffCaller.deploymentReadiness.backupRestoreDrill()
    ).resolves.toMatchObject({ destructiveRestoreScriptAvailable: false });
    await expect(
      staffCaller.deploymentReadiness.failureExercises()
  ...
  it("exposes a safe degraded-mode exercise shape without success claims", async () => {
    const caller = appRouter.createCaller(baseCtx("ops_admin") as unknown);
    const matrix = await caller.deploymentReadiness.failureExercises();
    expect(matrix.exercises).toHaveLength(8);
    expect(
      matrix.exercises.every(
        exercise =>
          typeof exercise.degradedSignal === "string" &&
          typeof exercise.manualFallback === "string"
      )
    ).toBe(true);
    expect(
      matrix.exercises.find(
        exercise => exercise.name === "db_connection_degradation"
      )
  ...
  it("health/readiness outputs are safe and do not leak secrets", async () => {
    ...
    const caller = appRouter.createCaller(baseCtx("ops_admin") as unknown);
    const [live, drill] = await Promise.all([
      caller.deploymentReadiness.liveness(),
      caller.deploymentReadiness.backupRestoreDrill(),
    ]);
  ...
  it("restore drill refuses destructive execution and redacts credentials", () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["scripts/restore-db-drill.mjs", "--execute", "--backup-file", backup],
      {
        env: {
          ...process.env,
          RESTORE_DATABASE_URL:
            "mysql://user:secret@nonprod.local/test_restore",
        },
        encoding: "utf8",
      }
    );
  ...
  it("does not claim fake deployment proof", async () => {
    const caller = appRouter.createCaller(baseCtx("super_admin") as unknown);
    const summary = await caller.deploymentReadiness.summary();
    expect(JSON.stringify(summary).toLowerCase()).toContain(
      "no production deployment proof is asserted"
    );
    expect(JSON.stringify(summary).toLowerCase()).not.toContain(
      "production verified"
    );
  });
