...
export const providerContracts: readonly ProviderContract[] = [
  {
    ...
    notes:
      "Production payment success requires real Razorpay verification. Env presence means configured only, not production-ready or verified.",
  },
  {
    ...
    manualInterventionEvents: [
      "whatsapp_send_dead_letter",
      "regulated_whatsapp_intent",
    ...
    notes:
      "WhatsApp Cloud API send failures should retry and dead-letter after max attempts; regulated flows stay staff/pharmacist gated.",
  },
  {
    providerName: "otp",
    ...
    manualInterventionEvents: [
      ...
      "otp_rate_limit_backend_invalid",
    ],
    opsDashboardStatuses: [
      "configured",
      "provider_unconfigured",
    ...
    notes:
      "Production OTP requires provider credentials and an explicit production-safe rate-limit backend; dev codes must not appear in production.",
  },
  {
    providerName: "sms",
    domain: "messaging",
    ...
    notes:
      "MSG91 SMS sends may be retried; unconfigured/demo paths are never sent.",
  },
  {
    providerName: "email",
    domain: "messaging",
    ...
    notes:
      "No dedicated runtime connector was found in this branch; contract reserves disabled-safe behavior until an adapter is wired.",
  },
  {
    ...
    notes:
      "No dedicated push connector was found in this branch; missing provider must degrade safely instead of claiming delivery.",
  },
  {
    providerName: "ocr",
    ...
    notes:
      "OCR output is assistive only; purchase/stock mutation requires reviewed approval/commit and must not auto-commit from provider output.",
  },
  {
    ...
    notes:
      "Sensitive prescription/file access must stay authenticated, audited, and fail closed when storage provider configuration is missing.",
  },
  {
    ...
    manualInterventionEvents: [
      "geocode_failed",
      "delivery_distance_manual_override",
    ],
    opsDashboardStatuses: [
      "configured",
      "provider_unconfigured",
    ...
    notes:
      "Maps/distance functionality is optional; unconfigured provider must require manual distance/routing rather than inventing a calculation.",
  },
  {
    ...
    notes:
      "Tally CSV/export generation is distinct from ERP sync/import. Generated/exported is not synced unless a real ERP response confirms it.",
  },
  {
    ...
    notes:
      "Printer unavailability may generate preview/ZPL/manual fallback but must never be labelled printed without a real print path.",
  },
] as const;

export const providerContractNames = providerContracts.map(
  contract => contract.providerName
);
