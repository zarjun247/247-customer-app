...
describe("deployment proof scripts", () => {
  it("does not print secret values during production env validation", () => {
    ...
    const result = runScript(
      ...
      ["--mode", "production"],
      {
        ...
        APP_ENV: "production",
        DATABASE_URL:
          "mysql://app_user:db-password@db.internal:3306/customer_app",
        JWT_SECRET: secret,
  ...
  it("detects duplicate migration numbers", () => {
    ...
    fs.writeFileSync(
      path.join(dir, "0001_second.sql"),
      "CREATE TABLE b (id int);\n"
    );
    const result = runScript("scripts/verify-migrations.mjs", [
      "--migrations-dir",
      dir,
    ]);
    expect(result.status).not.toBe(0);
  ...
  it("restore refuses a production-looking target", () => {
    ...
    const result = runScript(
      ...
      ["--dry-run", "--backup-file", backup],
      {
        RESTORE_DATABASE_URL:
          "mysql://restore_user:pw@prod-db.internal:3306/customer_app_prod",
        APP_ENV: "test",
      }
    );
    expect(result.status).not.toBe(0);
  ...
  it("restore verification is read-only, validates checksum, and redacts credentials", () => {
    ...
    const result = runScript(
      ...
      {
        RESTORE_DATABASE_URL:
          "mysql://restore_user:secret-password@restore-db.internal:3306/customer_app_restore",
        APP_ENV: "staging",
      }
    );
    const output = `${result.stdout}${result.stderr}`;
  ...
  it("restore verification refuses production-looking targets and execute flags", () => {
    ...
    const result = runScript(
      "scripts/restore-verify.mjs",
      ["--execute", "--backup-file", backup],
      {
        RESTORE_DATABASE_URL:
          "mysql://restore_user:secret@primary-prod-db.internal:3306/customer_app",
        APP_ENV: "production",
      }
    );
    expect(result.status).not.toBe(0);
  ...
  it("deployment env validation enforces staging URL, artifact identity, and unsafe flag posture without leaking secrets", () => {
    const result = runScript(
      ...
      ["--env", "staging"],
      {
        ...
        DATABASE_URL:
          "mysql://app_user:secret-db-password@staging-db.internal:3306/customer_app",
        GIT_SHA: "abc123",
  ...
  it("release gate summarizes blockers and exits nonzero on critical failure", () => {
    ...
    const result = runScript(
      ...
      ["--mode", "production", "--artifact-dir", dir],
      {
        NODE_ENV: "production",
        APP_ENV: "production",
        DATABASE_URL: "mysql://app@db.internal:3306/customer_app",
    ...
    const report = fs.readFileSync(
      path.join(dir, "RELEASE_GATE_REPORT.md"),
      "utf8"
    );
