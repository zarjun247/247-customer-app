...
describe('reports and reconciliation read-model guards', () => {
  ...
  it('H1 completeness report flags missing fields while tolerating current-main field names', () => {
    ...
    expect(report.rows[0].missingFlags).toEqual(expect.arrayContaining(['billNo', 'patientPhone', 'batchNo', 'prescriptionRef', 'saleLineRef']));
  ...
  it('supplier outstanding report returns current payable totals', () => {
    const report = buildSupplierOutstandingReport([
      { supplierId: 1, supplierName: 'Supplier', storeId: 10, invoiceTotal: '500.00', paymentTotal: '125.00', returnCreditTotal: '25.00', committedInvoiceCount: 2 },
    ]);
  ...
  it('daily sale GST report returns rows, totals, and csvData', () => {
    const report = buildDailyGstReport([
      { date: '2026-05-07', storeId: 10, taxableValue: '90.00', gstAmount: '10.00', grossSales: '100.00', discounts: 0, refundsOrReturns: 0, invoiceCount: 1 },
    ]);
  ...
  it('report router reconciliation procedures are read-only for stock/payment/invoice state', () => {
    ...
    for (const procedure of ['stockReconciliation', 'paymentInvoiceConsistency', 'supplierOutstanding', 'dailySaleGst']) {
      const start = source.indexOf(`${procedure}: protectedProcedure`);
      const next = source.indexOf(': protectedProcedure', start + procedure.length);
...
describe('audit reference hygiene guards', () => {
  it('scanner catches Number(uuid), Number(line.id), Number(saleId), and entityId: 0 patterns', () => {
    const sample = 'entityId: 0; entityId: Number(uuid); Number(line.id); Number(saleId);';
    expect(scanUnsafeAuditReferencePatterns(sample)).toEqual(expect.arrayContaining(['entityId: 0', 'Number(uuid)', 'Number(line.id)', 'Number(saleId)']));
  });

  it('blocks unsafe audit reference casts in current H1/statutory report paths', () => {
    const files = ['server/services/complianceGate.ts', 'server/routers/reportsRouter.ts'];
    const findings = files.flatMap((file) => scanUnsafeAuditReferencePatterns(fs.readFileSync(file, 'utf8')).map((match) => `${file}: ${match}`));
...
describe('sensitive report/audit payload redaction guard', () => {
  it('redacts prescription image data, OTPs, payment signatures/secrets, and auth cookies/tokens', () => {
    const raw = 'otp=123456 signature=pay_sig secret=topsecret Authorization: Bearer abc.def.ghi Cookie: auth=token123 prescriptionImage=data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAAB';
  ...
  it('redacts report payload objects before audit/log exposure', () => {
    const redacted = redactReportPayload({ otp: '123456', gatewaySignature: 'sig_secret', cookie: 'session=rawtoken', image: 'data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD' });
