...
describe("supplier ageing and reconciliation", () => {
    ...
    expect(report.rows).toMatchObject([
      {
        ...
        paidAmount: 100,
        outstandingAmount: 0,
        reconciliationStatus: "internal_settled",
      },
      {
        purchaseInvoiceId: 102,
        invoiceAmount: 200,
        ...
        reconciliationStatus: "internal_open",
      },
    ]);
  ...
  it("keeps store balances isolated when caller filters invoice inputs", () => {
    const report = buildSupplierReconciliationReport({
      ...
      allocations: [
        {
          supplierPaymentId: 902,
          purchaseInvoiceId: 401,
          amount: "30.00",
          allocationType: "invoice_payment",
        },
        {
          supplierPaymentId: 903,
  ...
  it("reconciles invoice minus allocations and credits without fake external statuses", () => {
    const report = buildSupplierReconciliationReport({
      ...
      allocations: [
        {
          supplierPaymentId: 904,
          purchaseInvoiceId: 501,
          amount: "250.00",
          allocationType: "invoice_payment",
        },
        {
          supplierPaymentId: 905,
          purchaseInvoiceId: 501,
          amount: "50.00",
          allocationType: "debit_note",
        },
    ...
    expect(report.rows[0].outstandingAmount).toBe(
      report.rows[0].invoiceAmount - report.rows[0].allocatedAmount
    );
    expect(report.rows[0].advances).toBe(50);
    expect(["internal_open", "internal_settled"]).toContain(
      report.rows[0].reconciliationStatus
    );
