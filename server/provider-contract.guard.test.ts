...
describe("provider contract matrix guards", () => {
  it("has one contract entry for every known external provider", () => {
    expect(
      getAllProviderContracts()
        .map(contract => contract.providerName)
        .sort()
    ).toEqual([...knownProviders].sort());
    expect(
      new Set(providerContracts.map(contract => contract.providerName)).size
    ).toBe(providerContracts.length);
  });

  it("requires production-required providers to list required env vars and fail closed or manual safe", () => {
    const productionRequired = getAllProviderContracts().filter(
      contract => contract.productionRequired
    );
    expect(
      productionRequired.map(contract => contract.providerName).sort()
    ).toEqual(["object_storage", "otp", "razorpay_payment"]);
  ...
  it("does not allow unconfigured production providers to be evaluated as success", () => {
    ...
    expect(
      statuses.find(status => status.providerName === "razorpay_payment")
    ).toMatchObject({
    ...
    expect(
      statuses.find(status => status.providerName === "whatsapp")
    ).toMatchObject({
      status: "provider_unconfigured",
      configured: false,
    });
    expect(
      statuses.find(status => status.providerName === "sms")
    ).toMatchObject({
      status: "provider_unconfigured",
      configured: false,
    });
    expect(
      statuses.find(status => status.providerName === "tally_erp_export")
    ).toMatchObject({
      status: "provider_unconfigured",
      configured: false,
    });
  ...
  it("does not label demo/test provider outcomes as sent, printed, synced, or verified", () => {
    ...
    expect(
      demoStatuses.find(status => status.providerName === "razorpay_payment")
        ?.status
    ).toBe("demo_skipped");
    expect(
      demoStatuses.find(status => status.providerName === "whatsapp")?.status
    ).toBe("demo_skipped");
    expect(
      demoStatuses.find(
        status => status.providerName === "printer_label_printing"
      )?.status
    ).toBe("preview_only");
  });
  ...
  it("documents payment, WhatsApp, SMS, printer, and ERP/Tally fail-closed status distinctions", () => {
    expect(getProviderContract("razorpay_payment").successStates).toEqual([
      "verified",
    ]);
    ...
    expect(getProviderContract("printer_label_printing").failureMode).toBe(
      "preview_only"
    );
    expect(
      getProviderContract("printer_label_printing").unavailableStates
    ).toEqual(expect.arrayContaining(["preview_only", "not_printed"]));
    expect(getProviderContract("tally_erp_export").successStates).toEqual(
      expect.arrayContaining(["export_generated", "synced"])
    );
    expect(getProviderContract("tally_erp_export").unavailableStates).toContain(
  ...
  it("marks OCR as human-review/manual-only before purchase or stock mutation", () => {
    ...
    expect(contract.unavailableStates).toEqual(
      expect.arrayContaining(["pending_manual_review", "manual_only"])
    );
    expect(contract.manualInterventionEvents).toEqual(
      expect.arrayContaining(["draft_pending_review"])
    );
  ...
  it("marks storage sensitive-file access as audit-required and fail-closed", () => {
    ...
    expect(contract.requiredEnvVars).toEqual([
      "BUILT_IN_FORGE_API_URL",
      "BUILT_IN_FORGE_API_KEY",
    ]);
    expect(contract.manualInterventionEvents).toContain(
  ...
  it("defines retry/dead-letter/manual-intervention requirements for operational providers", () => {
    for (const name of [
      ...
      expect(contract.opsDashboardStatuses).toEqual(
        expect.arrayContaining(["retry_scheduled", "dead_letter"])
      );
  ...
  it("status helper exposes configured/unconfigured/disabled without raw secret values", () => {
    ...
    expect(
      reports.find(report => report.providerName === "razorpay_payment")
    ).toMatchObject({
      status: "configured",
      configured: true,
      missingEnvVars: [],
    ...
    expect(
      reports.find(report => report.providerName === "whatsapp")
    ).toMatchObject({
      status: "disabled",
      enabled: false,
    });
