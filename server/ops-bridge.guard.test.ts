...
describe('ops bridge static guards', () => {
  ...
  it('shift closing should use reconciliationTruth', () => {
    const s = fs.existsSync('server/services/shiftClosing.ts') ? fs.readFileSync('server/services/shiftClosing.ts','utf8') : '';
  ...
  it('ops reports include rows/totals/csvData shape in reports router', () => {
    ...
    expect(s.includes('rows')).toBe(true); expect(s.includes('totals')).toBe(true); expect(s.includes('csvData')).toBe(true);
  });
});
