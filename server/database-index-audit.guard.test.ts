...
const schemaSql = readdirSync(join(drizzleDir, "schema"))
  .filter(f => f.endsWith(".ts") && f !== "index.ts")
  .map(f => readFileSync(join(drizzleDir, "schema", f), "utf8"))
  .join("\n");
const statusDoc = readFileSync(join(repoRoot, "docs", "STATUS.md"), "utf8");

const indexNames = [...migrationSql.matchAll(/ADD\s+INDEX\s+`([^`]+)`/gi)].map(
  match => match[1]
);
...
const requiredCoverage = [
  {
    area: "products/barcodes",
    indexes: [
      "idx_products_canonical_name",
  ...
  {
    area: "batchLedger/stock movements",
    indexes: [
  ...
  {
    area: "invoices/bill numbers",
    indexes: [
      "uq_sales_bill_no",
      "idx_invoice_snapshots_bill_no",
      "credit_notes_credit_note_no_unique",
    ],
  },
  {
    area: "payments/refunds",
    indexes: ["idx_counter_payments_gateway_ref", "idx_refunds_sale_status"],
  },
  {
    area: "H1 register",
    indexes: ["idx_h1_register_store_created", "idx_h1_register_bill_no"],
  },
  {
    area: "audit logs",
    indexes: ["idx_audit_logs_entity_created", "idx_audit_logs_action_created"],
  },
];

describe("database index audit migration guards", () => {
  it("does not duplicate a migration number", () => {
    const numbered = readdirSync(drizzleDir).filter(file =>
      /^\d{4}_.*\.sql$/.test(file)
    );
    ...
    expect(numbered.filter(file => file.startsWith("0044_"))).toEqual([
      migrationName,
    ]);
  });
  ...
  it("keeps migration statements limited to additive secondary indexes", () => {
    ...
    for (const statement of statements) {
      ...
      expect(statement).not.toMatch(
        /DROP|DELETE|UPDATE|INSERT|MODIFY|CHANGE|ADD\s+CONSTRAINT|FOREIGN\s+KEY/i
      );
    }
  });

  it("mirrors new migration index names in drizzle schema metadata", () => {
    const missingFromSchema = indexNames.filter(
      name => !schemaIndexNames.includes(name)
    );
  ...
  it("covers critical query families with indexes or documented deferrals", () => {
    ...
    for (const coverage of requiredCoverage) {
      const hasIndexOrDeferral =
        coverage.indexes.some(indexName =>
          combinedEvidence.includes(indexName)
  ...
  it("documents that empirical proof is required before performance and deployment claims", () => {
    ...
    expect(statusDoc).toMatch(
      /Hosted DB observation still required|no production deployment.*proof claimed/i
    );
  });
});
