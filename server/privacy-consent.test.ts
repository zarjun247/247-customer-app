...
import {
  buildSensitiveAccessAuditEvent,
  maskEmail,
  maskPhone,
  redactSensitiveForLogs,
} from "./services/sensitiveDataPolicy";
...
describe("privacy consent and sensitive data policy", () => {
  ...
  it("can grant and revoke consent while preserving an audit trail", async () => {
    ...
    await grantConsent(
      { userId: 7, phone: "+919999999999" },
      "refill_reminder",
      { changedBy: 7, source: "app" },
      store
    );
    expect(
      await getConsentStatus({ userId: 7 }, "refill_reminder", store)
    ).toBe("granted");
    ...
    const trail = await getConsentAuditTrail(
      { userId: 7 },
      "refill_reminder",
      store
    );
  ...
  it("blocks marketing, refill, dosage, and family profile access unless explicit consent is granted", async () => {
    ...
    expect(
      isReminderOrMarketingAllowed(
        await getConsentStatus({ userId: 8 }, "whatsapp_marketing", store),
        "whatsapp_marketing"
      )
    ).toBe(false);
    expect(
      isReminderOrMarketingAllowed(
        await getConsentStatus({ userId: 8 }, "dosage_reminder", store),
        "dosage_reminder"
      )
    ).toBe(false);
    await expect(
      assertConsentForSensitiveAction({ userId: 8 }, "refill_reminder", store)
    ).rejects.toBeInstanceOf(ConsentRequiredError);
    await expect(
      assertFamilyProfileAccessConsent({ userId: 8 }, store)
    ).rejects.toThrow(/family_profile_access/);

    await grantConsent(
      { userId: 8 },
      "family_profile_access",
      { source: "staff", changedBy: 44 },
      store
    );
    await expect(
      assertFamilyProfileAccessConsent({ userId: 8 }, store)
    ).resolves.toEqual({
      consentType: "family_profile_access",
      status: "granted",
    });
  });
  ...
  it("keeps transactional notification treatment explicit and separate from reminders or marketing", () => {
    expect(
      isTransactionalNotificationAllowed("pending", "whatsapp_transactional")
    ).toBe(true);
    expect(
      isTransactionalNotificationAllowed("revoked", "sms_transactional")
    ).toBe(false);
    expect(isReminderOrMarketingAllowed("pending", "refill_reminder")).toBe(
      false
    );
    expect(isReminderOrMarketingAllowed("revoked", "sms_marketing")).toBe(
      false
    );
    expect(isReminderOrMarketingAllowed("granted", "dosage_reminder")).toBe(
      true
    );
  });
