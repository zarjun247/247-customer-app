...
describe("payment gateway guards", () => {
  ...
  it("fails closed when webhook enabled without implemented route marker", () => {
    ...
    expect(errors).toContain(
      "PAYMENT_WEBHOOK_ENABLED_UNSUPPORTED_WITHOUT_VERIFIED_ROUTE"
    );
  });
  ...
  it("returns false for malformed or unequal-length webhook signatures instead of throwing", () => {
    ...
    expect(() =>
      verifyGatewayWebhookSignature("{}", "zz-not-hex")
    ).not.toThrow();
  ...
  it("cannot verify payment when Razorpay secret is missing in production", async () => {
    ...
    await expect(
      paymentConnector.verifyPayment({
        gatewayOrderId: "o_1",
        gatewayPaymentId: "p_1",
        signature: "sig",
      })
    ).rejects.toThrow();
    await expect(
  ...
  it("lifecycle helpers reject missing gatewayOrderId with BAD_REQUEST", async () => {
    await expect(recordPaymentAttempt({})).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
    await expect(markPaymentAuthorized({})).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
    await expect(markPaymentRefunded({})).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
  });
  ...
  it("recordPaymentAttempt is wired to DB (not a NOT_IMPLEMENTED stub)", async () => {
    ...
    await expect(
      recordPaymentAttempt({ gatewayOrderId: "order_guard_1" })
  ...
  it("markPaymentAuthorized is wired to DB (not a NOT_IMPLEMENTED stub)", async () => {
    await expect(
      markPaymentAuthorized({
        gatewayOrderId: "order_guard_2",
        gatewayPaymentId: "pay_guard_2",
      })
    ).rejects.toMatchObject({ code: "INTERNAL_SERVER_ERROR" });
  });
  ...
  it("markPaymentRefunded is wired to DB (not a NOT_IMPLEMENTED stub)", async () => {
    await expect(
      markPaymentRefunded({
        gatewayOrderId: "order_guard_3",
        providerRefundId: "rfnd_guard_3",
      })
    ).rejects.toMatchObject({ code: "INTERNAL_SERVER_ERROR" });
  });
