...
import {
  deadLetterProviderEventInMemory,
  scheduleProviderRetryInMemory,
  type ProviderRetryProofState,
} from "./services/providerEventsService";
...
describe("provider dead-letter retry proof", () => {
  it("schedules provider retry by incrementing attempt count without emitting fake success", () => {
    const state: ProviderRetryProofState = {
      providerEventId: 7,
    ...
    const retry = scheduleProviderRetryInMemory(state, {
      reason: "refund ledger temporarily unavailable",
    });
  ...
  it("moves exhausted provider event to exactly one dead letter and preserves operator review fields", () => {
    ...
    const first = deadLetterProviderEventInMemory(state, {
    ...
    const reviewed = {
      ...
      deadLetter: {
        ...first.deadLetter!,
        reviewedBy: "ops-1",
        reviewedAt: new Date("2026-05-10T00:00:00.000Z"),
        reviewNote: "review in progress",
      },
    };
    const second = deadLetterProviderEventInMemory(reviewed, {
      reason: "second worker race",
      deadLetterClass: "max_retries_exceeded",
    });
  ...
  it("schema and migration enforce one dead letter per provider event", () => {
    ...
    const migration = fs.readFileSync(
      "drizzle/0049_provider_retry_dead_letters.sql",
      "utf8"
    );
