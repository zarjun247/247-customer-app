...
describe('store isolation static guards', () => {
  it('has central rbac helpers', () => {
    ...
    const found = searchInFiles(files, /requireStoreAccess|requireStaffStore|assertCanCrossStore/);
    expect(found.length).toBeGreaterThan(0);
  });

  it('high risk routers include active store access checks', () => {
    const routers = ['server/routers/inventoryRouter.ts','server/routers/purchaseRouter.ts','server/routers/salesRouter.ts','server/routers/reportsRouter.ts','server/routers/deliveryRouter.ts'];
  ...
  it('blocks default store fallback patterns', () => {
    const dirs = [...listTsFiles('server/routers'), ...listTsFiles('server/_core')];
    const files = readFiles(dirs);
    const found = searchInFiles(files, /storeId\s*\?\?\s*1|storeId\s*\|\|\s*1|staffStoreId\s*\?\?\s*1|staffStoreId\s*\|\|\s*1/);
    expect(found).toEqual([]);
  });

  it('fails if requireStoreAccess import is unused in touched routers', () => {
    const targets = ['server/routers/deliveryRouter.ts','server/routers/reportsRouter.ts'];
    const bad: string[] = [];
