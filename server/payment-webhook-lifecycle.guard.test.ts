...
import fs from "fs";
import { verifyGatewayPaymentSignature, verifyGatewayWebhookSignature } from "./services/paymentGateway";
import { extractProviderWebhookRefs, hashRawPayload, sanitizeProviderWebhookPayload, verificationStatusIsCommerciallySafe } from "./services/paymentWebhookLifecycle";

const service = fs.readFileSync("server/services/paymentWebhookLifecycle.ts", "utf8");
const route = fs.readFileSync("server/paymentWebhookRoutes.ts", "utf8");
const schema = fs.readFileSync("drizzle/schema.ts", "utf8");
const migration = fs.readFileSync("drizzle/0045_provider_webhook_events.sql", "utf8");
...
describe("payment webhook lifecycle safety", () => {
  ...
  it("fails closed when production webhook secret or raw body is missing", () => {
    ...
    expect(service).toContain('Raw webhook body is required for signature verification');
    expect(route).toContain('rawBodyFromRequest(req)');
    process.env.PAYMENT_WEBHOOK_ENABLED = "true";
    process.env.RAZORPAY_WEBHOOK_SECRET = "";
    process.env.NODE_ENV = "production";
    expect(() => verifyGatewayWebhookSignature("{}", "abc")).toThrow(/Webhook secret missing/);
  });
  ...
  it("detects duplicate provider event IDs and duplicate payload hashes before processing", () => {
    expect(service).toContain("findExistingEvent(PROVIDER, refs.providerEventId, rawPayloadHash)");
  ...
  it("sanitizes raw payloads and does not expose signatures, tokens, instruments, PII, or prescription data", () => {
    const payload = {
      ...
      payload: { payment: { entity: { id: "pay_1", order_id: "order_1", email: "a@example.com", contact: "9999999999", card: { last4: "1111" }, notes: { orderId: "7", prescription: "rx" } } } },
      token: "secret-token",
    };
    ...
    expect(extractProviderWebhookRefs(payload)).toMatchObject({ eventType: "payment.captured", providerEventId: "evt_1", gatewayOrderId: "order_1", gatewayPaymentId: "pay_1", orderId: 7 });
  });

  it("appends lifecycle/audit events when a commercial event ledger is available", () => {
    for (const event of ["payment_webhook_received", "payment_signature_verified", "payment_captured", "payment_failed", "refund_webhook_received", "refund_completed", "payment_duplicate_event_ignored"]) {
