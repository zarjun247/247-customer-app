...
export const purchaseRouter = router({
  ...
  createInvoice: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await writeAuditLog({ actor: { id: ctx.user!.id, role: ctx.user!.role ?? undefined, type: "user" }, action: "create", entityType: "purchase_invoice", entityId: id, before: null, after: { invoiceNo: input.invoiceNo }, reason: "purchase invoice created", channel: "app" });
      return { id };
    }),
  ...
  updateInvoice: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await writeAuditLog({ actor: { id: ctx.user!.id, role: ctx.user!.role ?? undefined, type: "user" }, action: "update", entityType: "purchase_invoice", entityId: input.id, before: inv, after: updates, reason: "purchase invoice updated", channel: "app" });
      return { success: true };
    }),
  ...
  cancelInvoice: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await writeAuditLog({ actor: { id: ctx.user!.id, role: ctx.user!.role ?? undefined, type: "user" }, action: "cancel", entityType: "purchase_invoice", entityId: input.id, before: inv, after: { status: "cancelled" }, reason: input.reason, channel: "app" });
      return { success: true };
    }),
  ...
  commitInvoice: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await writeAuditLog({ actor: { id: ctx.user!.id, role: ctx.user!.role ?? undefined, type: "user" }, action: "commit", entityType: "purchase_invoice", entityId: input.id, before: { status: "draft" }, after: { status: "committed", gstSummary }, reason: "purchase commit", channel: "app" });
  ...
  createReturn: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const id = (result as { insertId: number }).insertId;
      await writeAuditLog({ actor: { id: ctx.user!.id, role: ctx.user!.role ?? undefined, type: "user" }, action: "create", entityType: "purchase_return", entityId: id, before: null, after: { purchaseInvoiceId: input.purchaseInvoiceId }, reason: input.reason, channel: "app" });
      return { id };
    }),

  addReturnLine: protectedProcedure
    .input(z.object({ purchaseReturnId: z.number(), purchaseLineId: z.number(), batchId: z.number(), qty: z.number().min(1), returnRate: z.string(), reason: z.string().min(1) }))
    .mutation(async ({ ctx, input }) => {
      requirePurchase(ctx.user!.role);
      const db = await getDbSafe();
  ...
  commitReturn: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await writeAuditLog({ actor: { id: ctx.user!.id, role: ctx.user!.role ?? undefined, type: "user" }, action: "commit", entityType: "purchase_return", entityId: input.id, before: { status: "draft" }, after: { status: "committed" }, reason: "purchase return committed", channel: "app" });
      return { success: true };
    }),
  ...
  recordPayment: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const id = (result as { insertId: number }).insertId;
      await writeAuditLog({ actor: { id: ctx.user!.id, role: ctx.user!.role ?? undefined, type: "user" }, action: "create", entityType: "supplier_payment", entityId: id, before: null, after: { supplierId: input.supplierId, amount: input.amount }, reason: "supplier payment created", channel: "app" });
      return { id };
    }),
