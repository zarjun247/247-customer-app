...
function requireCtxUser(ctx: { user?: { id?: number; role?: string; staffStoreId?: number | null } | null }) {
  if (!ctx.user) throw new TRPCError({ code: "UNAUTHORIZED", message: "Authentication required" });
  return ctx.user;
}

function toCommandContext(ctx: { user?: { id?: number; role?: string; staffStoreId?: number | null } | null; requestId?: string }): CommandContext {
  const user = ctx.user;
  return {
...
export const reservationRouter = router({
  ...
  reserve: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const user = requireCtxUser(ctx as { user?: { id?: number; role?: string; staffStoreId?: number } | null });
      const context = toCommandContext(ctx as typeof ctx & { requestId?: string });
