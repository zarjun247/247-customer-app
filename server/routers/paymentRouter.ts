...
import { createGatewayOrder, verifyGatewayPayment, markPaymentCaptured, markPaymentFailed, getPaymentByGatewayOrder } from "../services/paymentGateway";
import {
  getPaymentByOrderId,
  createSlaEvent,
  closeSlaEvent,
...
export const paymentRouter = router({
  ...
  verifyPayment: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const verification = await verifyGatewayPayment({ gatewayOrderId: input.gatewayOrderId, gatewayPaymentId: input.gatewayPaymentId, signature: input.signature });
      if (!verification.verified || !verification.realGatewayVerification) {
        await logAudit({ action: "payment.verify_failed", entityType: "payment", entityId: null, afterJson: { gatewayOrderId: input.gatewayOrderId, status: verification.status, reason: verification.reason } }, ctx);
        const code = verification.status === "provider_unconfigured" ? "PRECONDITION_FAILED" : "BAD_REQUEST";
        throw new TRPCError({ code, message: verification.reason ?? "Payment signature verification failed" });
      }
