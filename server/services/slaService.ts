...
export function computeOrderSla(t: Timeline, config: SlaConfig = SLA_DEFAULTS) {
  const targetMins = t.inBuilding
    ? config.inBuildingMins
    : t.urgent
      ? config.urgentMins
      : config.standardMins;
  ...
  return {
    createdToAccepted,
    acceptedToPacked,
    packedToOutForDelivery,
    outForDeliveryToDelivered,
    totalOrderSla,
    targetMins,
    breached,
    breachReason: breached ? "total_order_sla_exceeded" : null,
  };
}
