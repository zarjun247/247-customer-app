...
beforeEach(() => {
  mockGetDb.mockResolvedValue(
    makeCustomerDb([]) as unknown as Awaited<ReturnType<typeof getDb>>
  );
});
...
describe("getCustomerRefillRisks", () => {
  ...
  it("minRiskLevel filters out entries below threshold", async () => {
    ...
    expect(
      result.every(r => ["medium", "high", "critical"].includes(r.riskLevel))
    ).toBe(true);
...
describe("getStoreRefillAlerts", () => {
  ...
  it("limit option caps the results", async () => {
    ...
    mockGetDb.mockResolvedValue(
      makeStoreDb(rows) as unknown as Awaited<ReturnType<typeof getDb>>
    );
