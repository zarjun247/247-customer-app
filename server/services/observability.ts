...
const SENSITIVE_KEY_PATTERN =
  /(authorization|cookie|set-cookie|password|passwd|pwd|secret|token|session|jwt|otp|code|api[_-]?key|access[_-]?key|private[_-]?key|client[_-]?secret|signature|razorpay[_-]?signature|razorpay[_-]?secret|whatsapp.*token|whatsapp.*secret|database[_-]?url|db[_-]?url|payment.*secret|payment.*signature)/i;
const PHI_KEY_PATTERN =
  /(prescription|medical|diagnosis|symptom|notes?|doctor|patient|rx|h1|schedule[_-]?h|phone|email|address|flatNumber|userAddress)/i;
const BINARY_KEY_PATTERN =
  /(buffer|blob|base64|image|imageData|file|fileData|document|documentData|rawBody|rawPayload|ocrRawText|prescriptionImage)/i;
...
export function redactString(value: string): string {
  return value
    ...
    .replace(
      /(razorpay_signature|signature|secret|token|password|otp|api[_-]?key|database_url)(["'\s:=]+)([^\s,"'}]+)/gi,
      `$1$2${REDACTED}`
    )
...
export function redactValue(value: unknown, depth = 0, keyHint = ""): unknown {
  ...
  if (typeof value === "string") {
    ...
    if (redacted.length > MAX_STRING_LENGTH)
      return `[REDACTED:length:${redacted.length}]`;
    return redacted;
  }
  ...
  if (Array.isArray(value))
    return value
      .slice(0, 50)
      .map(item => redactValue(item, depth + 1, keyHint));
