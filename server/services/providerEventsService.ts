...
const RETRY_TERMINAL_STATUSES = new Set([
  "processed",
  "ignored_duplicate",
  "unsupported_event",
]);
const DEAD_LETTER_DUPLICATE_CODE = new Set([
  "ER_DUP_ENTRY",
...
export function scheduleProviderRetryInMemory(
  state: ProviderRetryProofState,
  input: { reason: string }
) {
  if (state.processingStatus === "dead_letter" || state.deadLetter)
    return { ...state, emittedSuccess: false as const };
  if (state.processingStatus === "processed")
    return { ...state, emittedSuccess: false as const };
  return {
    ...state,
...
export function deadLetterProviderEventInMemory(
  state: ProviderRetryProofState,
  input: { reason: string; deadLetterClass?: string }
) {
  if (state.deadLetter)
    return {
      ...state,
      ...
      emittedSuccess: false as const,
      duplicateDeadLetter: true as const,
    };
  return {
    ...state,
    processingStatus: "dead_letter" as const,
...
export async function scheduleProviderEventRetry(
  ...
  if (
    RETRY_TERMINAL_STATUSES.has(event.processingStatus) ||
  ...
  if (nextAttempt >= maxAttempts) {
    const dead = await moveProviderEventToDeadLetterOnce(db, {
      providerEvent: event,
      reason: input.reason,
      deadLetterClass: "max_retries_exceeded",
    ...
    return {
      ...
      attemptCount: nextAttempt,
      scheduled: false,
      deadLetter: dead.deadLetter,
      duplicate: dead.duplicate,
      emittedSuccess: false as const,
    };
  }
...
export async function moveProviderEventToDeadLetterOnce(
  ...
  const [existing] = await db
    ...
    .where(eq(providerDeadLetters.providerEventId, input.providerEvent.id))
    .limit(1);
  if (existing) {
    await db
      .update(providerWebhookEvents)
      .set({ processingStatus: "dead_letter", failureReason: input.reason })
  ...
  try {
    const [result] = await db.insert(providerDeadLetters).values({
      ...
      failureReason: input.reason,
      attemptCount:
        input.attemptCount ?? Number(input.providerEvent.attemptCount ?? 0),
      deadLetterClass: input.deadLetterClass ?? "max_retries_exceeded",
