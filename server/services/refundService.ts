...
import {
  ...
  REFUND_CONSUMING_STATUSES,
  type RefundProviderState,
  type RefundLedgerStatus,
} from "./refundHelpers";

export type { RefundProviderState, RefundLedgerStatus } from "./refundHelpers";
export { calculateRefundAvailability } from "./refundHelpers";
...
export async function getRefundTotalByPayment(paymentId: number) {
  ...
  const [row] = await db
    .select({
      totalPaise: sql<number>`coalesce(sum(${refunds.amountPaise}), 0)`,
    })
    .from(refunds)
    .where(
      and(
        eq(refunds.paymentId, paymentId),
        inArray(refunds.status, REFUND_CONSUMING_STATUSES)
      )
    );
...
async function getSuccessfulRefundTotalByPayment(paymentId: number) {
  ...
  const [row] = await db
    ...
    .from(refunds)
    .where(
      and(eq(refunds.paymentId, paymentId), eq(refunds.status, "success"))
    );
  return Number(row?.totalPaise ?? 0);
}

export async function assertRefundIdempotent(input: {
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        eq(refunds.providerRefundId, input.refundId)
      )
    )
    .limit(1);
  if (row) {
    await logAudit(
      {
        action: "refund.duplicate_detected",
        entityType: "refund",
        entityId: row.id,
        afterJson: {
...
export async function assertProviderRefundIdAvailable(input: {
  provider: string;
  ...
  if (existing)
    throw new TRPCError({
      code: "CONFLICT",
      message: "Duplicate provider refund",
    });
}

export async function assertRefundAmountAllowed(input: {
  gatewayOrderId: string;
  amountPaise: number;
}) {
  ...
  return calculateRefundAvailability({
...
export async function createRefundRecord(input: {
  ...
  if (input.providerRefundId)
    await assertProviderRefundIdAvailable({
      provider,
...
export async function initiateRefundRecord(input: {
  ...
  await logAudit(
    {
      ...
      entityType: "refund",
      entityId: created.refundId,
      afterJson: {
        gatewayOrderId: input.gatewayOrderId,
        amountPaise: input.amountPaise,
        provider: input.provider ?? DEFAULT_REFUND_PROVIDER,
      },
    },
    input.ctx
  );
  await appendCommercialEventBestEffort({
    ...
    idempotencyKey: input.providerRefundId
      ? `refund_initiated:${input.providerRefundId}`
      : `refund:${created.refundId}:initiated`,
    correlationId: input.gatewayOrderId,
  });
  return created;
}

export async function markRefundSuccess(input: {
  refundId: number;
  providerRefundId?: string | null;
  provider?: string;
  ctx?: CtxLike;
  ...
  await db
    ...
    .where(eq(refunds.id, input.refundId));
  const successfulTotal = await getSuccessfulRefundTotalByPayment(
    refund.paymentId
  );
  ...
  const fullyRefunded = payment
    ? successfulTotal >= Number(payment.amount ?? 0)
    : false;
  await db
    .update(paymentRecords)
    .set({
      refundId: input.providerRefundId ?? refund.providerRefundId,
      refundedAt: fullyRefunded ? new Date() : (payment?.refundedAt ?? null),
  ...
  await appendCommercialEventBestEffort({
    ...
    idempotencyKey: input.providerRefundId
      ? `refund_completed:${input.providerRefundId}`
      : `refund:${input.refundId}:completed`,
    correlationId: payment?.gatewayOrderId ?? null,
  });
}

export async function markRefundFailedRecord(input: {
  refundId: number;
  reason: string;
  providerRefundId?: string | null;
  provider?: string;
...
export async function initiateRefund(input: {
  ...
  if (!providerConfigured) {
    await logAudit(
      {
        action: "refund.provider_pending",
        entityType: "refund",
        entityId: created.refundId,
        afterJson: {
          gatewayOrderId: input.gatewayOrderId,
    ...
    return {
      ...
      amountPaise: input.amountPaise,
      providerRefundId: input.refundId ?? null,
      providerState: "provider_not_configured" as RefundProviderState,
    };
  }
  ...
  if (!gatewayPaymentId) {
    await markRefundFailedRecord({
      refundId: created.refundId,
      reason: "Payment gateway payment id missing",
      ctx: input.ctx,
    });
    return {
      ok: false,
      ...
      amountPaise: input.amountPaise,
      providerRefundId: input.refundId ?? null,
      providerState: "manual_required" as RefundProviderState,
    };
  }

  try {
    const providerRefund = await paymentConnector.refund({
      gatewayPaymentId,
    ...
    if (["processed", "succeeded", "success"].includes(providerStatus)) {
      ...
      return {
        ...
        amountPaise: input.amountPaise,
        providerRefundId: providerRefund.refundId,
        providerState: "succeeded" as RefundProviderState,
      };
    }
    await logAudit(
      {
        ...
        afterJson: {
          gatewayOrderId: input.gatewayOrderId,
          providerState: "pending_provider",
          providerStatus,
        },
      },
      input.ctx
    ...
    return {
      ...
      amountPaise: input.amountPaise,
      providerRefundId: providerRefund.refundId,
      providerState: "pending_provider" as RefundProviderState,
    };
  } catch (error) {
    const reason =
      error instanceof Error ? error.message : "Refund provider call failed";
    await markRefundFailedRecord({
      refundId: created.refundId,
      reason,
      ctx: input.ctx,
    ...
    return {
      ...
      amountPaise: input.amountPaise,
      providerRefundId: input.refundId ?? null,
      providerState: "failed" as RefundProviderState,
...
export async function recordRefund(input: {
  ...
  const status: RefundLedgerStatus =
    input.providerState === "failed" ||
    input.providerState === "manual_required"
      ? "failed"
  ...
  const refundRecord =
    ...
    (
      await createRefundRecord({
        gatewayOrderId: input.gatewayOrderId,
        providerRefundId: input.refundId,
        amountPaise: (await getPaymentByGatewayOrderId(input.gatewayOrderId))
          .amount,
        status: "pending",
      })
    ).refundId;
  await markRefundSuccess({
    refundId: typeof refundRecord === "number" ? refundRecord : refundRecord.id,
    providerRefundId: input.refundId,
    ctx: input.ctx,
  });
}
...
export async function markRefundFailed(input: {
  ...
  if (input.refundId) {
    const [existing] = await db
      ...
      .where(
        and(
          eq(refunds.provider, DEFAULT_REFUND_PROVIDER),
          eq(refunds.providerRefundId, input.refundId)
        )
      )
      .limit(1);
  ...
  if (!refundRecordId) {
    ...
    const created = await createRefundRecord({
      gatewayOrderId: input.gatewayOrderId,
      amountPaise: Math.max(1, Number(payment.amount ?? 0)),
      providerRefundId: input.refundId ?? null,
      status: "pending",
    });
    refundRecordId = created.refundId;
  }
  return markRefundFailedRecord({
