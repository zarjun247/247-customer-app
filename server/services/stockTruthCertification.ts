...
export type StockMutationViolation = {
  path: string;
  line: number;
  pattern: string;
  text: string;
...
export function isApprovedStockMutationPath(
  path: string,
  allowlist: readonly string[] = DEFAULT_ALLOWED_PATHS
) {
  const clean = normalized(path);
  return allowlist.some(
    allowed => clean === allowed || clean.startsWith(allowed)
  );
}

export function scanStockMutationContent(
  files: StockMutationScanFile[],
  allowlist: readonly string[] = DEFAULT_ALLOWED_PATHS
): StockMutationViolation[] {
...
export function collectStockMutationScanFiles(
  rootDir = process.cwd()
): StockMutationScanFile[] {
...
export function calculateAvailability(input: CanonicalAvailabilityInput) {
  ...
  const quarantined =
    Number(input.quarantined ?? 0) + Number(input.unavailable ?? 0);
  ...
  return {
    ...
    appVisibleAvailable:
      input.appVisibleAvailable == null
        ? Math.max(0, rawAvailable)
        : Number(input.appVisibleAvailable),
    formula:
...
function expiryTime(expiryDate: string | Date) {
  return new Date(expiryDate).getTime();
}
function isExpired(batch: FefoBatch, asOf: Date) {
  return (
    expiryTime(batch.expiryDate) <= asOf.getTime() ||
    (batch.expired ?? 0) > 0 ||
    batch.status === "expired"
  );
}
function isQuarantined(batch: FefoBatch) {
  return (
    (batch.quarantined ?? 0) > 0 ||
    ...
    batches
      .map(batch => ({
        ...
        availability: calculateAvailability({
          onHand: batch.onHand,
          activeReserved: batch.activeReserved,
          quarantined: batch.quarantined,
          unavailable: batch.unavailable,
          expired: batch.expired,
          blocked: batch.blocked ? batch.onHand : 0,
        }),
      }))
      .filter(
        ({ batch, availability }) =>
          !isExpired(batch, asOf) &&
          !isQuarantined(batch) &&
          availability.calculatedAvailable >= qty
      )
      .sort(
        (a, b) =>
          expiryTime(a.batch.expiryDate) - expiryTime(b.batch.expiryDate)
      )[0]?.batch ?? null
  );
}

export function assertFefoPick(input: {
  pickedBatchId: string | number;
  batches: FefoBatch[];
  qty?: number;
  asOf?: Date;
  overrideReason?: string | null;
}) {
  const expected = selectFefoBatch(input.batches, {
    qty: input.qty,
    asOf: input.asOf,
  });
  if (!expected)
    return {
      ok: false,
      deviationType: "no_valid_batch",
      expectedBatchId: null,
      requiresAudit: true,
    };
  if (String(expected.batchId) === String(input.pickedBatchId))
    return {
      ok: true,
      deviationType: null,
      expectedBatchId: expected.batchId,
      requiresAudit: false,
    };
  return {
    ok: Boolean(input.overrideReason),
    deviationType: "manual_override",
    expectedBatchId: expected.batchId,
    requiresAudit: !input.overrideReason,
  };
}

export function buildFefoDeviationReport(
  picks: Array<{
    pickedBatchId: string | number;
    batches: FefoBatch[];
    qty?: number;
  ...
  return {
    ...
    totals: {
      ...
      deviationCount: rows.filter(row => !row.result.ok).length,
      overrideAuditRequired: rows.filter(row => row.result.requiresAudit)
        .length,
    },
    csvData: rows,
  };
...
export function buildStockTruthReconciliationReport(
  inputRows: StockTruthReconciliationInputRow[]
) {
  const rows = inputRows.map(row => {
    const availability = calculateAvailability({
      ...
      blocked: row.blocked,
      expired: row.expired,
      appVisibleAvailable: row.appVisibleAvailable,
    });
    const ledgerMovementTotal = Number(
      row.ledgerMovementTotal ?? row.onHand ?? 0
    );
    ...
    if (Number(row.activeReserved ?? 0) > Number(row.onHand ?? 0))
      anomalyType.push("reserved_exceeds_on_hand");
    if (varianceQty !== 0) anomalyType.push("batch_ledger_mismatch");
    if (availability.appVisibleAvailable !== availability.calculatedAvailable)
      anomalyType.push("store_sku_mismatch");
    if (Number(row.expired ?? 0) > 0 && availability.appVisibleAvailable > 0)
      anomalyType.push("expired_marked_available");
    if (
      Number(row.quarantined ?? 0) + Number(row.unavailable ?? 0) > 0 &&
      availability.appVisibleAvailable > 0
    )
      anomalyType.push("quarantined_marked_available");
    if (row.batchId == null && row.batchLedgerId == null)
      anomalyType.push("missing_batch_ref");
    if (row.directMutationSuspected)
      anomalyType.push("direct_mutation_suspected");
    return {
