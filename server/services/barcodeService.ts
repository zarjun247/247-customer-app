...
export function generateInternalBarcode(seed: {
  productId: number;
  batchId?: number;
  storeId?: number;
}) {
...
export async function assertBarcodeUnique(barcode: string) {
  ...
  const [a] = await db
    .select()
    .from(barcodeAliases)
    .where(eq(barcodeAliases.barcode, code))
    .limit(1);
  const [p] = await db
    .select()
...
export async function registerBarcodeAlias(input: {
  barcode: string;
  productId?: number;
  batchId?: number;
  storeId?: number;
  ...
  await assertBarcodeUnique(barcode);
  const [row] = await db.insert(barcodeAliases).values({
    ...input,
    barcode,
    isActive: true,
...
export async function resolveBarcode(barcodeInput: string, storeId?: number) {
  ...
  const [alias] = await db
    ...
    .where(
      and(
        eq(barcodeAliases.barcode, barcode),
        eq(barcodeAliases.isActive, true)
      )
    )
    .limit(1);
  ...
  const q = db
    .select({
      ...
      batchId: batchLedger.id,
      batchNo: batchLedger.batchNo,
      expiryDate: batchLedger.expiryDate,
      mrp: batchLedger.mrp,
      batchAvailableQty: sql<number>`${batchLedger.qtyOnHand} - ${batchLedger.qtyReserved} - ${batchLedger.qtyQuarantined} - ${batchLedger.qtyExpired}`,
    })
    .from(products)
    .leftJoin(batchLedger, eq(batchLedger.productId, products.id))
    .where(
      productId
        ? eq(products.id, productId)
        : or(
            eq(products.barcode, barcode),
            eq(batchLedger.internalBarcode, barcode),
            eq(batchLedger.manufacturerBarcode, barcode)
          )!
    );
  const rows = await q;
  const canonicalRows = await Promise.all(
    rows.map(async row => {
      if (!storeId || !row.productId)
        return {
          ...row,
          availableQty: Math.max(0, Number(row.batchAvailableQty ?? 0)),
        };
...
export function getBarcodeLabelPayload(input: {
  productName: string;
  ...
  return {
    ...
    masterValidation,
    status: masterValidation.status,
  };
}
export async function createLabelPrintJob(input: {
  barcodeAliasId?: number | null;
  ...
  const db = await getDb();
  if (!db) return null;
  const [r] = await db.insert(labelPrintJobs).values({
    ...input,
...
export async function reprintLabel(id: number) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(labelPrintJobs)
    .set({ status: "queued", error: null, printedAt: null })
    .where(eq(labelPrintJobs.id, id));
}
export async function getLabelPrintQueue(storeId?: number) {
  const db = await getDb();
  if (!db) return [];
  return db
    .select()
    .from(labelPrintJobs)
    .where(storeId ? eq(labelPrintJobs.storeId, storeId) : sql`1=1`)
    .orderBy(asc(labelPrintJobs.createdAt));
}
export async function markLabelPrinted(id: number, error?: string | null) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(labelPrintJobs)
    .set({
      status: error ? "failed" : "printed",
      error: error ?? null,
      printedAt: error ? null : new Date(),
    })
    .where(eq(labelPrintJobs.id, id));
}
export function recordScanEvent(): Promise<void> {
