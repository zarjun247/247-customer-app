...
export async function getLineCostBasis(productId: string, batchLedgerId?: string | null) {
  const db = await getDb(); const { batchLedger, productSupplierMappings } = await import("../../drizzle/schema");
  if (batchLedgerId) { const [b] = await db.select().from(batchLedger).where(eq(batchLedger.id, Number(batchLedgerId))).limit(1); if (b) return Number(b.landingCost ?? b.purchaseRate ?? 0); }
  const [m] = await db.select().from(productSupplierMappings).where(eq(productSupplierMappings.productId, Number(productId))).orderBy(desc(productSupplierMappings.id)).limit(1);
  return Number(m?.lastPurchaseRate ?? 0);
}
export async function calculateLineGrossMargin(line: any) { const cost = await getLineCostBasis(String(line.productId), line.batchLedgerId); const revenue = Number(line.saleRate) * Number(line.qty) - Number(line.discountAmount ?? 0); const cogs = cost * Number(line.qty); const marginAmt = revenue - cogs; const marginPct = revenue > 0 ? (marginAmt / revenue) * 100 : 0; return { costPerUnit: cost, revenue, cogs, marginAmt, marginPct }; }
export async function calculateSaleMargin(lines: any[]) { const vals = await Promise.all(lines.map(calculateLineGrossMargin)); const revenue = vals.reduce((a, v) => a + v.revenue, 0); const cogs = vals.reduce((a, v) => a + v.cogs, 0); const marginAmt = revenue - cogs; return { revenue, cogs, marginAmt, marginPct: revenue > 0 ? (marginAmt / revenue) * 100 : 0 }; }
...
export async function validateDiscountCode(code: string, sale: any) {
  ...
  if (dc.startsAt && now < new Date(dc.startsAt)) return { valid: false, reason: "Discount code not started" };
  if (dc.endsAt && now > new Date(dc.endsAt)) return { valid: false, reason: "Discount code expired" };
  if (dc.usageLimit !== null && dc.usageLimit !== undefined && (dc.usedCount ?? 0) >= dc.usageLimit) return { valid: false, reason: "Discount code usage limit reached" };
  if (Number(sale.subtotal ?? 0) < Number(dc.minOrderAmount ?? 0)) return { valid: false, reason: "Minimum order not met" };
  return { valid: true, code: dc };
}

export async function applyDiscountCode(saleId: string, code: string, ctx?: any) {
  const db = await getDb(); const { sales, discountCodes } = await import("../../drizzle/schema");
  ...
  if (dc.maxDiscountAmount !== null && dc.maxDiscountAmount !== undefined) discount = Math.min(discount, Number(dc.maxDiscountAmount));
