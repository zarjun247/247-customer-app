...
export function defaultTallyLedgerMapping(
  row: Pick<
    TallyLedgerSourceRow,
    "sourceType" | "accountCode" | "accountName" | "metadataJson"
  >
): string {
  const haystack = [
    row.sourceType,
    row.accountCode,
    row.accountName,
    row.metadataJson?.ledgerType,
    row.metadataJson?.gstType,
  ]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
  if (
    /gst.*input|input.*gst|cgst.*input|sgst.*input|igst.*input/.test(haystack)
  )
    return "Input GST";
  if (
    /gst.*output|output.*gst|cgst.*output|sgst.*output|igst.*output/.test(
      haystack
    )
  )
    return "Output GST";
...
export function buildTallyCsvRows(
  rows: TallyLedgerSourceRow[],
  input: { storeId?: number | null; companyIdentifier?: string | null } = {}
): TallyCsvRow[] {
  return rows.map((row, index) => {
    ...
    if (Number(debitAmount) === 0 && Number(creditAmount) === 0) {
      throw new Error(
        `Tally export row ${row.id ?? index} has neither debit nor credit amount`
      );
    }
    const sourceReference = [
      row.sourceType ?? "journal",
      row.sourceId ?? row.id ?? index,
    ].join(":");
    ...
    return {
      ...
      ledgerName:
        row.accountName || row.accountCode || defaultTallyLedgerMapping(row),
      debitAmount,
      creditAmount,
...
export function toTallyCsv(rows: TallyCsvRow[]) {
  return [
    CSV_HEADERS.join(","),
    ...rows.map(row =>
      CSV_HEADERS.map(header => JSON.stringify(row[header] ?? "")).join(",")
    ),
  ].join("\n");
}
...
export async function generateTallyCsvExport(
  ...
  const idempotencyKey = `tally:export:${buildDuplicateKey({ storeId: input.storeId ?? null, exportType: input.exportType, periodStart, periodEnd, checksum: deterministicSha256({ rows: input.rows, exportType: input.exportType }) })}`;
  await recordProviderAttempt({
    providerType: "tally",
    operationType: "export",
    entityType: "tally_export",
    entityRef: idempotencyKey,
  ...
  try {
    const tallyRows = buildTallyCsvRows(input.rows, {
    ...
    if (existing) {
      await markProviderCompleted({
        ...
        userId: input.generatedBy ?? null,
        idempotencyKey,
        providerRef: String(existing.id),
        responsePayload: {
          duplicate: true,
          checksum,
      ...
      return {
        ...
        rows: tallyRows,
        csv,
        providerState: "export_generated_duplicate_not_synced",
        imported: false,
        synced: false,
      };
    }
    ...
    await markProviderCompleted({
      ...
      userId: input.generatedBy ?? null,
      idempotencyKey,
      providerRef: String(run.id),
      responsePayload: { checksum, rowCount: tallyRows.length },
    });
    await markProviderManualRequired({
      providerType: "tally",
      operationType: "sync",
      entityType: "tally_export",
      entityRef: String(run.id),
      storeId: input.storeId ?? null,
      userId: input.generatedBy ?? null,
      idempotencyKey: `${idempotencyKey}:sync`,
      lastErrorMessage:
        "Tally CSV generated locally; ERP import/sync confirmation is manual_required",
    });
    return {
      ...
      rows: tallyRows,
      csv,
      providerState: "provider_unconfigured_export_generated",
      imported: false,
      synced: false,
    };
  } catch (error: any) {
    ...
    return {
      ...
      runId: run.id,
      failureReason,
      providerState: "export_generation_failed_not_synced",
      imported: false,
      synced: false,
    };
  }
...
export async function getGstNetPosition(db: any) {
  ...
  return {
    rows: [{ outputRows: out.length, inputRows: inp.length }],
    totals: {
      outputRows: out.length,
      inputRows: inp.length,
      netRows: out.length - inp.length,
    },
    csvData: [
      {
        outputRows: out.length,
