...
type DrizzleOrTx = DbInstance | DbTx;
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        eq(invoiceSequences.documentType, docType)
      )
    )
    .limit(1);
...
export async function assertInvoiceNumberUnique(
  db: DrizzleOrTx,
  value: string
) {
...
async function reserveInvoiceNumberOnce(
  ...
  const financialYear = financialYearFromIndiaBusinessDate(date);
  const prefix = prefixFor(storeId, docType, date);
  const reserve = async (tx: DrizzleOrTx) => {
    ...
    const sequence = Number(
      (rows as unknown as Array<{ sequence?: number }>)[0]?.sequence ?? 0
    );
    return formatInvoiceNumber({ prefix, sequence });
  };
  if ("transaction" in db && typeof db.transaction === "function") {
    return (db as DbInstance).transaction(reserve);
  }
  return reserve(db);
}
...
export async function generateInvoiceNumber(db: DrizzleOrTx, storeId: string) {
...
export async function generateReturnNoteNumber(
  db: DrizzleOrTx,
  storeId: string
) {
  return reserveInvoiceNumber(db, storeId, "return_note");
}
export async function getInvoiceNumberForSale(db: DrizzleOrTx, saleId: string) {
