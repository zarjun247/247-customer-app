...
function notImplementedLifecycleResult(
  helperName: string
  ...
  if (isExplicitPaymentDemoMode() && !runtimeIsProduction()) {
    return {
      ...
      message: `${helperName} skipped in explicit demo/test mode`,
    };
  }
  throw new TRPCError({
    code: "NOT_IMPLEMENTED",
    message: `${helperName} is not wired to durable payment state`,
  });
}
...
export async function verifyGatewayPaymentSignature(input: {
  ...
  const idempotencyKey = `payment:verify:${input.gatewayOrderId}:${input.gatewayPaymentId}`;
  await recordProviderAttempt({
    providerType: "payment",
    operationType: "verify",
    entityType: "gateway_order",
    entityRef: input.gatewayOrderId,
    idempotencyKey,
    requestPayload: {
      gatewayOrderId: input.gatewayOrderId,
      gatewayPaymentId: input.gatewayPaymentId,
      signatureHash: "present",
    },
  });
  ...
  if (!secret) {
    ...
    await markProviderNotConfigured({
      ...
      entityRef: input.gatewayOrderId,
      idempotencyKey,
      lastErrorMessage: "Razorpay key secret missing",
    });
    return {
      verified: false,
  ...
  if (verified) {
    await markProviderSuccess({
      ...
      idempotencyKey,
      status: "verified",
      providerRef: input.gatewayPaymentId,
  ...
  await markProviderFailure({
    ...
    lastErrorMessage: "Payment signature mismatch",
    retryable: false,
  });
  return {
    verified: false,
...
export async function recordPaymentAttempt(
  ...
  return notImplementedLifecycleResult("recordPaymentAttempt");
}
export async function markPaymentAuthorized(
  _: any
): Promise<PaymentLifecycleResult> {
  return notImplementedLifecycleResult("markPaymentAuthorized");
}
...
export async function markPaymentRefunded(
  ...
  return notImplementedLifecycleResult("markPaymentRefunded");
}
