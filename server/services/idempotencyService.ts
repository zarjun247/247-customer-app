...
export function canonicalJson(payload: unknown) { return JSON.stringify(canonicalize(payload)); }
export function createMutationFingerprint(payload: unknown) { return createHash("sha256").update(canonicalJson(payload)).digest("hex"); }
...
async function fetchOperation(db: any, table: any, key: string, scope: string) {
  const [row] = await db.select().from(table).where(and(eq(table.key, key), eq(table.scope, scope))).limit(1);
  return row ?? null;
}
...
export async function beginIdempotentOperation(input: any) {
  ...
  try {
    ...
    await logAudit({
      ...
      afterJson: { key: input.key, scope: input.scope, entityRef: input.entityId == null ? null : String(input.entityId) },
    }, input.ctx);
    return created ? { ...created, __created: true } : null;
  } catch (error) {
    if (!isDuplicateKeyError(error)) throw error;
    const existing = await fetchOperation(db, table, input.key, input.scope);
    if (!existing) throw new TRPCError({ code: "CONFLICT", message: "Idempotency operation exists but could not be loaded" });
...
export async function getExistingIdempotentResult(key: string, scope: string) { const db = await getDb(); const table = await getTable(); if (!db) return null; return fetchOperation(db, table, key, scope); }
...
function assertRequestHashMatches(existing: IdempotencyRow, requestHash?: string | null) {
  if (existing.requestHash && requestHash && existing.requestHash !== requestHash) throw new TRPCError({ code: "CONFLICT", message: "Idempotency key reused with different payload" });
}

export async function withIdempotency<T>(params: any, fn: () => Promise<T>): Promise<T> {
  assertIdempotencyKeyPresent(params.key);
  const existing = await beginIdempotentOperation(params) as IdempotencyRow | null;
  if (!existing) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Unable to create idempotency operation" });
  ...
  if (existing.status === "completed") {
    await logAudit({
      ...
      afterJson: { key: params.key, scope: params.scope, entityRef: params.entityId == null ? null : String(params.entityId) },
    }, params.ctx);
    return existing.resultJson as T;
  }
  if (existing.status === "started" && !existing.__created) throw new TRPCError({ code: "CONFLICT", message: "Duplicate in-progress operation; retry after the first request finishes" });
  // Failed operations may be retried only with the same requestHash. A successful retry overwrites the failed state.
  try { const result = await fn(); await completeIdempotentOperation(params.key, params.scope, result, params.ctx); return result; } catch (e: any) { await failIdempotentOperation(params.key, params.scope, { message: e?.message ?? "error" }); throw e; }
}
