...
export function normalizeInboundWhatsappMessage(input:{phone:string;body?:string}) {
  const body = (input.body ?? '').trim();
  const triage = triageSupportIssue(body);
  return { ...input, body, intent: triage.needsHuman ? 'support_escalation' : 'order_or_support', escalateToHuman: triage.needsHuman };
}
