      ...
      from: (table: unknown) => ({
        where: (_cond?: unknown) => ({
          limit: (n: number) => {
            ...
            const isDefTable =
              (table as any)?._?.name === "capability_definitions" ||
...
vi.mock("./auditHashChain", () => ({
  appendChainedAudit: vi.fn(() =>
    Promise.resolve({ sequenceNumber: 1, rowHash: "abc" })
  ),
}));
...
describe("grantCapability", () => {
  ...
  it("throws CapabilityDefinitionNotFoundError for unknown capability", async () => {
    await expect(
      grantCapability({ userId: 1, capabilityName: "no.such.cap" })
    ).rejects.toBeInstanceOf(CapabilityDefinitionNotFoundError);
  });

  it("throws when db is unavailable", async () => {
    vi.mocked(getDb).mockResolvedValueOnce(null as unknown);
    await expect(
      grantCapability({ userId: 1, capabilityName: "audit.view" })
...
describe("revokeCapability", () => {
  ...
  it("throws ActiveGrantNotFoundError when no active grant exists", async () => {
    await expect(
      revokeCapability({ userId: 99, capabilityName: "audit.view" })
    ).rejects.toBeInstanceOf(ActiveGrantNotFoundError);
  });

  it("throws when db is unavailable", async () => {
    vi.mocked(getDb).mockResolvedValueOnce(null as unknown);
    await expect(
      revokeCapability({ userId: 1, capabilityName: "audit.view" })
    ).rejects.toThrow("DB unavailable");
  });
  ...
  it("returns only active non-expired grants for a user", async () => {
    ...
    const activeRows = [
      ...
      },
      {
        id: "g3",
        userId: "8",
        capabilityName: "inventory.adjust",
        grantedAt: now,
        expiresAt: past,
        revokedAt: null,
        grantedByUserId: null,
