...
import { getStockoutForecast, getProductStockoutRisk } from "./stockoutIntelligence";
...
function makeProductDb(totalQty: number, stockQty = 0, softLockedQty = 0) {
  ...
  const mockSelect = () => {
    ...
    return {
      from: () => ({
        where: () => Promise.resolve(stockQty > 0 ? [{ stockQty, softLockedQty }] : []),
      }),
    };
  };
...
beforeEach(() => {
  mockGetDb.mockResolvedValue(makeDemandDb([]) as unknown as Awaited<ReturnType<typeof getDb>>);
});
...
describe("getStockoutForecast", () => {
  ...
  it("excludes products with 0 or negative demand", async () => {
    mockGetDb.mockResolvedValue(makeDemandDb([
  ...
  it("daysToStockout = currentSellable / averageDailyDemand", async () => {
    ...
    ], [{ productId: 1, stockQty: 25, softLockedQty: 5 }]) as unknown as Awaited<ReturnType<typeof getDb>>);
  ...
  it("riskLevel 'critical' when daysToStockout < 3", async () => {
    ...
    mockGetDb.mockResolvedValue(makeDemandDb([
      { productId: "1", totalQty: 180 },
    ], [{ productId: 1, stockQty: 2, softLockedQty: 0 }]) as unknown as Awaited<ReturnType<typeof getDb>>);
    const result = await getStockoutForecast("store-1", { numericStoreId: 42 });
    expect(result[0].riskLevel).toBe("critical");
  });
  ...
  it("riskLevel 'high' when 3 ≤ daysToStockout < 7", async () => {
    ...
    mockGetDb.mockResolvedValue(makeDemandDb([
      { productId: "1", totalQty: 180 },
    ], [{ productId: 1, stockQty: 5, softLockedQty: 0 }]) as unknown as Awaited<ReturnType<typeof getDb>>);
    const result = await getStockoutForecast("store-1", { numericStoreId: 42 });
  ...
  it("riskLevel 'medium' when 7 ≤ daysToStockout < lookaheadDays(30)", async () => {
    ...
    mockGetDb.mockResolvedValue(makeDemandDb([
      { productId: "1", totalQty: 180 },
    ], [{ productId: 1, stockQty: 15, softLockedQty: 0 }]) as unknown as Awaited<ReturnType<typeof getDb>>);
    const result = await getStockoutForecast("store-1", { numericStoreId: 42 });
  ...
  it("riskLevel 'low' when daysToStockout ≥ lookaheadDays", async () => {
    ...
    mockGetDb.mockResolvedValue(makeDemandDb([
      { productId: "1", totalQty: 180 },
    ], [{ productId: 1, stockQty: 60, softLockedQty: 0 }]) as unknown as Awaited<ReturnType<typeof getDb>>);
    const result = await getStockoutForecast("store-1", { numericStoreId: 42 });
  ...
  it("softLockedQty is subtracted from stockQty", async () => {
    ...
    mockGetDb.mockResolvedValue(makeDemandDb([
      { productId: "1", totalQty: 180 },
    ], [{ productId: 1, stockQty: 30, softLockedQty: 20 }]) as unknown as Awaited<ReturnType<typeof getDb>>);
    const result = await getStockoutForecast("store-1", { numericStoreId: 42 });
  ...
  it("forecastedStockoutDate is approx now + daysToStockout", async () => {
    ...
    expect(result[0].forecastedStockoutDate!.getTime()).toBeGreaterThanOrEqual(before + fivedays - 2000);
    expect(result[0].forecastedStockoutDate!.getTime()).toBeLessThanOrEqual(after + fivedays + 2000);
  });
  ...
  it("minRiskLevel filters out low-risk entries", async () => {
    ...
    const result = await getStockoutForecast("store-1", { numericStoreId: 42, minRiskLevel: "high" });
...
describe("getProductStockoutRisk", () => {
  ...
  it("0 demand → averageDailyDemand = 0, daysToStockout = -1 (infinite), riskLevel low", async () => {
    mockGetDb.mockResolvedValue(makeProductDb(0) as unknown as Awaited<ReturnType<typeof getDb>>);
  ...
  it("returns correct risk for product with known demand and stock", async () => {
    ...
    mockGetDb.mockResolvedValue(makeProductDb(180, 5, 0) as unknown as Awaited<ReturnType<typeof getDb>>);
    const result = await getProductStockoutRisk("p1", "store-1", { numericStoreId: 42 });
