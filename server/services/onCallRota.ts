...
export async function getCurrentOnCall(role: OnCallRole): Promise<OnCallShift | null> {
  const rota = readRota();
  const now = new Date().toISOString();
  return rota.shifts.find(s => s.role === role && s.startsAt <= now && s.endsAt >= now) ?? null;
}

export async function listUpcomingShifts(role: OnCallRole, days = 7): Promise<OnCallShift[]> {
  const rota = readRota();
  const now = new Date().toISOString();
  const cutoff = new Date(Date.now() + days * 86_400_000).toISOString();
  return rota.shifts.filter(s => s.role === role && s.endsAt >= now && s.startsAt <= cutoff);
}

export async function upsertShift(input: OnCallShift): Promise<void> {
  const rota = readRota();
...
export async function listAllShifts(): Promise<OnCallShift[]> {
  return readRota().shifts;
}
    ...
    await fetch("https://events.pagerduty.com/v2/enqueue", {
      ...
      body: JSON.stringify({
        ...
        payload: {
          ...
          severity: severity === "P0" ? "critical" : severity === "P1" ? "error" : "warning",
...
export async function escalate(
  ...
  logger.info({ event: "oncall_escalation", severity, role, context }, "oncall_escalation");

  const currentShift = await getCurrentOnCall(role);
  if (currentShift) {
    logger.info(
      { event: "oncall_escalation_target", role, primaryUserId: currentShift.primaryUserId },
      "oncall_escalation_target",
    );
  } else {
    logger.warn({ event: "oncall_no_shift_found", role }, "oncall_no_shift_found");
  }
