...
import { checkProviders, checkWorkerQueue, getHealthReport, publicReadiness, type HealthStatus } from "./healthcheck";
...
function noProofClaim(): RuntimeCheck {
  return {
    ...
    message: "No production deployment proof is asserted by this endpoint; validate with real CI/CD logs, runtime URL checks, backups, and observability.",
  };
}

function rollup(statuses: string[]): "ready" | "not_ready" {
  return statuses.some((status) => ["unhealthy", "not_ready", "unknown"].includes(status)) ? "not_ready" : "ready";
}

export async function getDeploymentReadinessSummary() {
  const [readiness, health, workerQueue] = await Promise.all([publicReadiness(), getHealthReport(), checkWorkerQueue()]);
...
export async function getWorkerQueueReadiness() {
  ...
  return safeMetadata({ status: workerQueue.status, generatedAt: new Date().toISOString(), workerQueue });
}

export function getDegradedModeStatus(...statuses: string[]) {
  const degraded = statuses.some((status) => ["degraded", "unhealthy", "unknown", "not_ready"].includes(status));
  return {
...
export function getBackupRestoreDrillReadiness() {
  return {
    ...
    restoreDrillScript: "scripts/restore-db-drill.mjs --dry-run --backup-file <non-production-backup.sql>",
    destructiveRestoreScriptAvailable: false,
    warning: "No production restore is automated. Restore drills require explicit non-production targets and refuse production-looking targets.",
  };
}
...
export async function getMultiStoreRuntimeOverview() {
  try {
    ...
    if (!db) return { status: "unknown" as const, message: "database_unavailable", generatedAt: new Date().toISOString() };
    const [storeCount] = await db.select({ value: count() }).from(stores);
    const [activeStoreCount] = await db.select({ value: count() }).from(stores).where(eq(stores.isActive, true));
    const [activeSkuCount] = await db.select({ value: count() }).from(storeSkus).where(eq(storeSkus.isActive, true));
    const [negativeStock] = await db.select({ value: count() }).from(storeSkus).where(lt(storeSkus.stockQty, 0));
    const [usersWithoutStore] = await db.select({ value: count() }).from(users).where(sql`${users.onboardingComplete} = true and ${users.assignedStoreId} is null`);
    const [ordersWithoutStore] = await db.select({ value: count() }).from(orders).where(sql`${orders.storeId} is null`);
    const isolation = Number(usersWithoutStore?.value ?? 0) === 0 && Number(ordersWithoutStore?.value ?? 0) === 0 && Number(negativeStock?.value ?? 0) === 0;
    return safeMetadata({
      status: isolation ? "healthy" : "degraded",
      generatedAt: new Date().toISOString(),
      stores: { total: Number(storeCount?.value ?? 0), active: Number(activeStoreCount?.value ?? 0) },
      inventory: { activeStoreSkuRows: Number(activeSkuCount?.value ?? 0), negativeStockRows: Number(negativeStock?.value ?? 0) },
      isolationChecks: { onboardedUsersWithoutAssignedStore: Number(usersWithoutStore?.value ?? 0), ordersWithoutStoreId: Number(ordersWithoutStore?.value ?? 0) },
    });
  } catch (error) {
    return { status: "unknown" as const, message: safeFailure(error), generatedAt: new Date().toISOString() };
  }
}
...
export async function getStoreRuntimeDetail(storeId: number) {
  try {
    const db = await getDb();
    if (!db) return { status: "unknown" as const, message: "database_unavailable", generatedAt: new Date().toISOString() };
    const [store] = await db.select({ id: stores.id, name: stores.name, isActive: stores.isActive, type: stores.type, pincode: stores.pincode, slaMins: stores.slaMins }).from(stores).where(eq(stores.id, storeId)).limit(1);
    if (!store) return { status: "unknown" as const, message: "store_not_found", generatedAt: new Date().toISOString() };
    const [skuRows] = await db.select({ value: count() }).from(storeSkus).where(eq(storeSkus.storeId, storeId));
    const [orderRows] = await db.select({ value: count() }).from(orders).where(eq(orders.storeId, storeId));
    return safeMetadata({ status: store.isActive ? "healthy" : "disabled", generatedAt: new Date().toISOString(), store, inventory: { skuRows: Number(skuRows?.value ?? 0) }, orders: { totalRows: Number(orderRows?.value ?? 0) } });
  } catch (error) {
    return { status: "unknown" as const, message: safeFailure(error), generatedAt: new Date().toISOString() };
  }
}
...
export async function getStoreIsolationChecks() {
  ...
  return safeMetadata({ ...(overview as Record<string, unknown>), generatedAt: new Date().toISOString(), policy: "Staff/admin only. Responses expose aggregate operational metadata only and never customer PHI/PII." });
}
