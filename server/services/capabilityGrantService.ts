...
export const CAPABILITY_ROLE_DEFAULTS: Readonly<Record<string, readonly string[]>> = {
...
export class ActiveGrantNotFoundError extends Error {
  constructor(userId: string | number, capabilityName: string) {
    super(`No active grant found for user=${userId}, capability=${capabilityName}`);
...
export async function grantCapability(params: GrantCapabilityParams): Promise<{ id: string }> {
  const db = await getDb();
  if (!db) throw new Error("DB unavailable for capability grant");

  const userIdStr = String(params.userId);
  const grantedByStr = params.grantedByUserId != null ? String(params.grantedByUserId) : null;
  ...
  await appendChainedAudit({
    ...
    entityId: id,
    actorUserId: params.grantedByUserId != null ? Number(params.grantedByUserId) : null,
    afterJson: { userId: userIdStr, capabilityName: params.capabilityName, expiresAt: params.expiresAt ?? null },
  });

  logger.info({ capabilityName: params.capabilityName, userId: userIdStr, msg: "capabilityGrant: granted" });
  return { id };
}

// ─── Revoke ───────────────────────────────────────────────────────────────────

export async function revokeCapability(params: RevokeCapabilityParams): Promise<{ id: string }> {
  const db = await getDb();
  if (!db) throw new Error("DB unavailable for capability revoke");

  const userIdStr = String(params.userId);
  const revokedByStr = params.revokedByUserId != null ? String(params.revokedByUserId) : null;
  ...
  if (!existing) throw new ActiveGrantNotFoundError(params.userId, params.capabilityName);

  const now = new Date();
  await db
    .update(capabilityGrants)
    .set({ revokedAt: now, revokedByUserId: revokedByStr, reason: params.reason ?? existing.reason })
  ...
  await appendChainedAudit({
    ...
    actorUserId: params.revokedByUserId != null ? Number(params.revokedByUserId) : null,
    afterJson: { userId: userIdStr, capabilityName: params.capabilityName, revokedAt: now.toISOString() },
  });

  logger.info({ capabilityName: params.capabilityName, userId: userIdStr, msg: "capabilityGrant: revoked" });
  return { id: existing.id };
}

// ─── Query ────────────────────────────────────────────────────────────────────

export function roleHasCapabilityDefault(role: string | null | undefined, capabilityName: string): boolean {
...
export async function hasCapability(
  ...
  if (userRole && roleHasCapabilityDefault(userRole, capabilityName)) return true;
  const db = await getDb();
  if (!db) return false;
...
export async function getUserCapabilities(
  ...
  return active as CapabilityGrantRow[];
}
...
export async function listGrants(params?: {
  ...
  const conditions = [];
  if (params?.userId != null) conditions.push(eq(capabilityGrants.userId, String(params.userId)));
  if (params?.capabilityName) conditions.push(eq(capabilityGrants.capabilityName, params.capabilityName));
  if (!params?.includeRevoked) conditions.push(isNull(capabilityGrants.revokedAt));

  const rows = conditions.length > 0
    ? await db.select().from(capabilityGrants).where(and(...conditions)).limit(limit)
    : await db.select().from(capabilityGrants).limit(limit);

  return rows as CapabilityGrantRow[];
}

export async function getCapabilityDefinitions(): Promise<Array<{ capabilityName: string; description: string; riskLevel: string }>> {
  const db = await getDb();
  if (!db) return [];
  const rows = await db.select().from(capabilityDefinitions).limit(1_000);
  return rows.map((r) => ({ capabilityName: r.capabilityName, description: r.description, riskLevel: r.riskLevel }));
}
