...
describe("staff acknowledgement and session security foundation", () => {
  ...
  it("records active staff confidentiality acknowledgements", async () => {
    ...
    expect(await hasActiveStaffAcknowledgement(41, "patient_data_confidentiality", "2026.05", store)).toBe(false);
    ...
    expect(await hasActiveStaffAcknowledgement(41, "patient_data_confidentiality", "2026.05", store)).toBe(true);
  ...
  it("lists and revokes only the targeted staff session without affecting unrelated users", async () => {
    const store = new InMemoryStaffSecurityStore();
    await recordStaffDeviceSession({ staffId: 1, sessionId: "sess-a", deviceId: "dev-a", terminalId: "cash-1" }, store);
    await recordStaffDeviceSession({ staffId: 2, sessionId: "sess-b", deviceId: "dev-b", terminalId: "cash-2" }, store);
    expect(await listActiveStaffSessions(undefined, store)).toHaveLength(2);

    const revoked = await revokeStaffSession("sess-a", 99, "lost terminal", store);
    expect(revoked?.staffId).toBe(1);
    expect(await listActiveStaffSessions(undefined, store)).toHaveLength(1);
    expect((await listActiveStaffSessions(undefined, store))[0].staffId).toBe(2);
    expect(store.audits.map((audit) => audit.action)).toContain("staff.session.revoked");
  });
  ...
  it("keeps timeout, terminal lock, cashier PIN, no shared account, and role switch policies explicit", () => {
    ...
    expect(STAFF_SESSION_POLICY.cashierPinRequiredForSensitiveActions).toContain("existing PIN/auth pattern");
    expect(STAFF_SESSION_POLICY.sharedSuperAdminAccounts).toContain("prohibited");
    expect(STAFF_SESSION_POLICY.roleSwitchPrevention).toContain("no in-session role elevation");
  });
});
