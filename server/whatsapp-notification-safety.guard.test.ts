...
describe("WhatsApp notification safety", () => {
      ...
      assertWhatsappWebhookGuard(
        {
          req: {
            header: name =>
              name === "x-webhook-token" ? "expected-token" : undefined,
          },
        },
        "{}"
      )
  ...
  it("accepts a valid WhatsApp webhook signature in production", () => {
    ...
    expect(validateWebhookSignature(payload, signature, "top-secret")).toBe(
      true
    );
    expect(() =>
      assertWhatsappWebhookGuard(
  ...
  it("detects regulated WhatsApp intents that must escalate to staff/pharmacist", () => {
    expect(
      isRegulatedMedicineIntent(
        "Can I change the dosage of my Schedule H refill? Please substitute it."
      )
    ).toBe(true);
    expect(
      isRegulatedMedicineIntent(
        "I have side effects after this Rx medicine, emergency help"
      )
    ).toBe(true);
  });
  ...
  it("supports sensitivity-only notification preference updates and idempotent defaults", () => {
    ...
    expect(src).toContain(
      "updates.allowSensitiveInUnsafeChannels !== undefined"
    );
    expect(
      src.match(/onDuplicateKeyUpdate/g)?.length ?? 0
    ).toBeGreaterThanOrEqual(3);
    expect(src).toContain("getNotificationPreferences(customerId)");
  });

  it("does not mark unavailable providers as sent", () => {
    expect(normalizeProviderResult(undefined).status).toBe(
      "provider_unconfigured"
    );
    expect(normalizeProviderResult(false).status).toBe("failed");
    expect(normalizeProviderResult({ ok: false, error: "boom" }).status).toBe(
      "failed"
    );
  });
