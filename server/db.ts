...
function canonicalAvailabilitySql() {
  return sql<number>`COALESCE((SELECT SUM(bl.qtyOnHand) FROM ${batchLedger} bl WHERE bl.productId = ${storeSkus.productId} AND bl.storeId = ${storeSkus.storeId} AND ((${storeSkus.variantId} IS NULL AND bl.variantId IS NULL) OR bl.variantId = ${storeSkus.variantId}) AND bl.status = 'active'), ${storeSkus.stockQty}) - COALESCE((SELECT SUM(COALESCE(sr.qty, sr.qtyReserved)) FROM ${stockReservations} sr WHERE sr.productId = ${storeSkus.productId} AND sr.storeId = ${storeSkus.storeId} AND ((${storeSkus.variantId} IS NULL AND sr.variantId IS NULL) OR sr.variantId = ${storeSkus.variantId}) AND sr.status = 'active' AND (sr.expiresAt IS NULL OR sr.expiresAt > NOW())), 0) - ${storeSkus.softLockedQty} - COALESCE((SELECT SUM(bl.qtyQuarantined + bl.qtyExpired + bl.qtyReserved) FROM ${batchLedger} bl WHERE bl.productId = ${storeSkus.productId} AND bl.storeId = ${storeSkus.storeId} AND ((${storeSkus.variantId} IS NULL AND bl.variantId IS NULL) OR bl.variantId = ${storeSkus.variantId}) AND bl.status = 'active'), 0)`;
}
