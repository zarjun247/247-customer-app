...
import fs from 'fs';
import { generateInternalBarcode, getBarcodeLabelPayload, normalizeBarcodeInput } from './services/barcodeService';

describe('barcode scan guards', () => {
  it('normalizes scanner wedge input', () => expect(normalizeBarcodeInput(' ab 12\n')).toBe('AB12'));
  it('deterministic internal barcode', () => expect(generateInternalBarcode({ productId: 1, batchId: 2, storeId: 3 })).toBe(generateInternalBarcode({ productId: 1, batchId: 2, storeId: 3 })));
  it('label payload excludes customer fields', () => {
    const payload = getBarcodeLabelPayload({ productName: 'X', internalBarcode: 'PHX-1' }) as any;
    expect(payload.customerName).toBeUndefined(); expect(payload.patientName).toBeUndefined();
  });
  it('scan routes do not call stockInvariant mutation directly', () => {
    ...
    expect(sales.includes('decreaseStockForSaleConfirmation') && sales.includes('confirmSale')).toBe(true);
  });
