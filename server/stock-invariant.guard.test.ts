...
describe("stock invariant guard", () => {
  ...
  it("blocks legacy movement helper usage in inventory router", () => {
    const found = /writeMovement\(/.test(
      read("server/routers/inventoryRouter.ts")
    );
    expect(found).toBe(false);
  });
  ...
  it("blocks direct stock movement inserts in migrated purchase/inventory flows", () => {
    ...
    const found =
      pattern.test(read("server/routers/purchaseRouter.ts")) ||
      pattern.test(read("server/routers/inventoryRouter.ts"));
    expect(found).toBe(false);
  });

  it("blocks direct qtyOnHand writes in inventory corrections path", () => {
    const pattern =
      /audit\.complete[\s\S]*update\(batchLedger\)\.set\(\{\s*qtyOnHand/;
