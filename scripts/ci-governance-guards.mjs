...
export function scanText(filePath, text) {
  ...
  scanLines(normalized, text, (line, lineNumber, lines) => {
    ...
    if (
      runtimePath &&
      /(provider_unconfigured|not_configured|disabled|manual_required)/i.test(
        providerWindow
      ) &&
      /(status\s*[:=]\s*["'`](sent|synced|verified|printed|paid|refunded|parsed|imported|complete|completed|success)["'`]|\b(sent|synced|verified|printed|paid|refunded|parsed|imported|complete|completed|success)\s*[:=]\s*true\b)/i.test(
        providerWindow
      ) &&
      ...
      addFinding(
        ...
        normalized,
        lineNumber,
        "Fail-closed provider status appears to be treated as sent/synced/verified/printed/paid/refunded/parsed/imported success.",
        line
      );
    }
    if (
      ...
      addFinding(
        ...
        normalized,
        lineNumber,
        "demo_skipped appears to be treated as real provider success.",
        line
      );
    }
    if (
