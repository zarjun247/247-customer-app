...
Reason: the repository shows strong software foundations, but live launch still lacks P0 evidence for provider verification, measured staging backup/restore, named operational ownership, staff access assignment, pharmacist SOP signoff, legal/compliance review, live monitoring ownership, hosted CI/deployment proof, and emergency rollback rehearsal.
...
| Hosted CI | Target branch checks archived; DB concurrency workflow observed if available. | Not observed in repository. |
...
| Hosted CI observation missing | P1 controlled rollout blocker | Local proof is useful but release branch parity is not archived. | Hosted workflow run links/log IDs for target branch. |
| Supplier invoice duplicate backfill/migration approval | P1 controlled rollout blocker if live purchasing is enabled; P2 scale blocker if purchasing remains manually controlled | Duplicate statutory/commercial purchase records can survive until reviewed. | Business-reviewed duplicate report, remediation decisions, and approved non-destructive uniqueness migration plan. |
